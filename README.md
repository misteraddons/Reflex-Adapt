...
Looking for the original Reflex Adapt Firmware? Check [here](https://github.com/misteraddons/Reflex-Adapt-Legacy).
...
3. **Automatic controller detection** - Auto input recognizes most supported
   ...
   detection prompts.
4. **Automatic host detection** - Auto output identifies supported Windows and
   console hosts. Generic USB hosts without a positive Windows or console
   signature, including MiSTer and typical Linux systems, use the saved
   MiSTer/Linux output preference.
...
System Settings contains global Windows and MiSTer/Linux Auto-output
preferences, display contrast, home-screen, Kiosk Mode, hotkey, sound, and
other device settings:

### Rumble behavior

Rumble defaults to **Variable** whenever the connected controller supports
proportional strength. **Variable** preserves the strength requested by the
host; **Max** converts each nonzero motor command to full strength while
keeping separate motor channels independent. DualShock and DualShock 2 pads
and SNES RumbleTech support variable strength. N64 Rumble Paks and wired
GameCube controllers are binary devices, so their menu is simply **Off/On**.
Rumble controls are hidden for controllers without supported feedback, such as
digital PSX pads, JogCon, WaveBird, and currently Dreamcast controllers.
...
| XInput (MiSTer/Linux or manual) | MiSTer, Linux | 2 | Two independent wired XInput controllers |
| XInput (Windows Auto) | Windows | 2 active / 4 slots | Four-child wireless-receiver personality; Classic2USB has two physical inputs |
...
With **Auto** output selected, **Windows Mode** and **MiSTer Mode** independently
choose DInput, XInput, or Keyboard. Both default to DInput. Windows XInput
uses the four-child receiver personality; MiSTer/Linux XInput uses two wired
interfaces and requires no MiSTer kernel changes. MiSTer Mode is the
generic-host fallback: it is used when detection does not find a positive
Windows or console signature, which includes MiSTer and typical Linux hosts but
can also include other generic USB hosts. DInput remains recommended because it
keeps WebHID, serial management, the utility drive, and MiSTer-specific paths
available.
