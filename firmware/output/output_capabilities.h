...
inline bool output_maps_n64_c_buttons_to_face_buttons() {
  ...
  if (output_runtime_is_switchpro_mode() && n64_cstick_mode == N64CSTICK_AUTO) {
    // A standard Pro Controller presents N64 C buttons through its right stick.
    return false;
  }
...
inline switchpro_mode_enum output_switch_profile_for_input_mode(DeviceEnum mode) {
  switch (mode) {
...
inline void output_apply_automatic_switch_profile_for_input_mode(DeviceEnum mode) {
  ...
  output_set_switch_profile(output_switch_profile_for_input_mode(mode));
}
