...
inline bool output_allows_management_msc(outputMode_t effectiveMode) {
  ...
  return false;
#else
  // Keep the virtual utility drive on every Windows/MiSTer DInput identity,
  // including the DInput stage while Auto is probing. Specialized MiSTer PSX
  // modes retain the same drive without inheriting the generic WebHID bundle.
  const bool genericDInputMode =
    output_is_generic_mister_hid_mode(effectiveMode) &&
    !output_is_console_clean_usb_mode(effectiveMode);
...
inline bool output_uses_switch_nes_profile() {
  return output_uses_switch_profile(SWITCHPRO_NES);
}

inline bool output_uses_switch_snes_profile() {
  return output_uses_switch_profile(SWITCHPRO_SNES);
}

inline bool output_uses_switch_genesis_profile() {
  return output_uses_switch_profile(SWITCHPRO_GENESIS);
}
...
inline bool output_maps_n64_c_buttons_to_face_buttons() {
  ...
  if (mode == OUTPUT_MISTER || mode == OUTPUT_HID) {
    // DInput defaults to four independent buttons, but Stick remains available.
    return get_effective_n64_cstick_mode() == N64CSTICK_AS_BUTTONS;
  }
  if (mode == OUTPUT_XINPUT || mode == OUTPUT_XINPUT2P) {
    // Auto and Buttons preserve four independent digital C-button controls.
    // Stick remains available as an explicit right-stick override.
    return get_effective_n64_cstick_mode() == N64CSTICK_AS_BUTTONS;
  }
  if (output_uses_switch_n64_profile()) {
    // Native NSO mapping needs all four C buttons on unique backing bits.
    return true;
  }
  return get_effective_n64_cstick_mode() == N64CSTICK_AS_BUTTONS;
}
...
inline bool output_should_spatialize_n64_c_buttons(const controller_state_t& frame) {
  ...
  if (mode == OUTPUT_XINPUT || mode == OUTPUT_XINPUT2P) {
    // XInput has enough digital controls to keep every N64 button unique.
    return false;
  }
  if (mode == OUTPUT_SWITCHPRO && output_uses_switch_n64_profile()) {
    return false;
  }
...
inline uint32_t output_apply_n64_c_buttons_to_face_buttons(uint32_t buttons,
  ...
  // X/Y are neutral backing bits for C-Up/C-Left. Remove all four backing
  // bits before placing the C buttons on their final face-button positions,
  // otherwise Name mapping makes C-Up and C-Left assert X and Y together.
...
inline uint32_t output_apply_n64_c_buttons_to_xinput_face_buttons(
    uint32_t buttons, const controller_state_t& frame) {
  if (!controllerFrameTypeNameIsN64(frame)) {
    return buttons;
  }
  ...
  // Adapt V1 XInput layout: the four C buttons occupy ABXY while physical
  // N64 A/B are moved to RT/LT by the transport mapping.
  buttons &= ~(kButtonA | kButtonB | kButtonX | kButtonY);
  if (get_effective_n64_cstick_mode() != N64CSTICK_AS_BUTTONS) {
    return buttons;
  }
...
inline uint8_t output_n64_c_backing_r2(const controller_state_t& frame) {
  return output_n64_c_backing_r2(
    ...
    output_should_spatialize_n64_c_buttons(frame),
    frame.R2,
    frame.L3);
}
...
inline bool output_maps_vboy_right_dpad_to_face_buttons() {
  const outputMode_t mode = canonicalizeOutputMode(get_effective_output_mode());
  if (mode == OUTPUT_MISTER || mode == OUTPUT_HID) {
    // DInput can preserve all four right-D-pad directions as distinct buttons.
    return true;
  }
  if (mode == OUTPUT_XINPUT || mode == OUTPUT_XINPUT2P) {
    // XInput uses the right stick so Virtual Boy A/B remain Xbox A/B only.
    return false;
  }
  return get_effective_n64_cstick_mode() == N64CSTICK_AS_BUTTONS;
}
...
inline switchpro_mode_enum output_switch_profile_for_input_mode(
    DeviceEnum mode, bool nsoSpecialEnabled) {
  ...
  switch (mode) {
    ...
    case RZORD_NES:
      return SWITCHPRO_NES;
    #endif
    #ifdef ENABLE_INPUT_SNES
    case RZORD_SNES:
      return SWITCHPRO_SNES;
    #endif
...
inline void output_apply_automatic_switch_profile_for_input_mode(DeviceEnum mode) {
  if (!output_runtime_is_switchpro_mode()) {
    return;
  }

  output_set_switch_profile(
    output_switch_profile_for_input_mode(
