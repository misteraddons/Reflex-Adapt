...
inline void map_xinput_output_to_report(uint8_t port, xinput_report_t& report) {
  ...
  report.lb = n64XinputLayout ? frame.L1
                              : (isN64ZCombined() ? (frame.L1 || frame.L2) : frame.L1);
  report.rb = frame.R1;
  report.ls = n64XinputLayout ? 0 : frame.L3;
  report.rs = n64XinputLayout ? 0 : frame.R3;
  report.back = n64XinputLayout ? frame.L2 : frame.SELECT;
  ...
  if (n64XinputLayout) {
    report.lt = frame.B ? 0xff : 0x00;
    report.rt = frame.A ? 0xff : 0x00;
  } else if (has_analog_triggers && !force_digital_triggers) {
    report.lt = frame.ANALOG_L2;
    report.rt = frame.ANALOG_R2;
  } else {
    uint8_t lt_btn = isN64ZCombined() ? 0 : frame.L2;
    report.lt = lt_btn ? 0xff : 0x00;
    report.rt = frame.R2 ? 0xff : 0x00;
  }
}
