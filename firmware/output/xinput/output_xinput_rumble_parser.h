...
inline size_t xinputRumbleParseSize(size_t transferred, size_t endpoint_size) {
  ...
  // TinyUSB can report zero while still filling the fixed endpoint buffer.
  // Short nonzero packets are XInput initialization/LED traffic, not rumble;
  // never extend those into stale bytes left in the endpoint buffer.
  if (transferred == 0) {
