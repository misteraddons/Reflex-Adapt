...
void usb_detect_handle_edpt_clear_stall(uint8_t ep_addr) {
  if (can_run_usb_detection()) {
    // Switch docks clear the active HID IN/OUT endpoints during the generic
    // probe, but endpoint numbers can drift as the composite changes shape.
    // Keep this as a candidate signal only. The mounted-host resolver below
    // still gives Windows and other generic HID hosts time to identify
    // themselves before it accepts the Switch signature.
