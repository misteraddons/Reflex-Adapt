...
static void xinputw_init(void) {
    if (_xinput_dev) {
        _xinput_dev->reset_transport_state();
    }
}

static void xinputw_reset(uint8_t rhport) {
    (void)rhport;
    // Preserve desired physical child presence across a host bus reset, but
    // require each child to repeat the receiver handshake after reconfigure.
    if (_xinput_dev) {
        _xinput_dev->reset_transport_state();
    }
}
            ...
            if (xitf->_xinput_out_buffer[0] == 0x08 && xitf->_xinput_out_buffer[1] == 0x00 &&
                ...
                switch (xitf->info_state) {
                    ...
                    case UNKNOWN1: {
                        ...
                        if (_xinput_dev->sendRawReport(i, controller_info,
                                                      sizeof(controller_info))) {
                            xitf->info_state = UNKNOWN2;
                            xitf->connection_retry_deadline_us = 0;
                        }
                        break;
                    }
                    case UNKNOWN2:
                ...
                if (parseXinputRumblePacket(xitf->_xinput_out_buffer, parse_size, &rumble)) {
                    ...
                    const uint8_t sourcePort = xinputw_source_port_for_target(i);
                    if (sourcePort != XINPUTW_INVALID_PORT) {
    ...
    *info = xinputw_diag;
    ...
    if (_xinput_dev) {
        for (uint8_t i = 0; i < XINPUT_WIRELESS_CONTROLLERS; ++i) {
            ...
            info->desired_connected[i] =
                _xinput_dev->interfaces[i].connected ? 1 : 0;
            info->info_state[i] =
                static_cast<uint8_t>(_xinput_dev->interfaces[i].info_state);
        }
    }
}
