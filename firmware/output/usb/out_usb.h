...
void rumble_callback(uint8_t index, uint8_t left_large, uint8_t right_small) {
  // A nonzero host effect is gameplay activity. Wake the OLED before storing
  // the command so screensaver suppression cannot discard the first pulse.
  if ((left_large | right_small) != 0) {
    resetIdleTimer();
  }
  const bool extendShortLightPulse =
    output_is_generic_mister_hid_mode(output_effective_mode()) ||
    output_effective_mode_is_any(OUTPUT_XINPUT, OUTPUT_XINPUT2P) ||
    output_effective_mode_is(OUTPUT_XINPUTW);
  rumbleRuntimeSetHostFeedback(
    index, left_large, right_small, extendShortLightPulse);
}
...
#ifdef ADAPT_OUTPUT_USB_DEVICE
// USB HID object: descriptor, length, protocol, interval, and OUT endpoint.
