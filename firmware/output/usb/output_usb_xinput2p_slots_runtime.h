...
#include "../xinputw/output_xinputw_slot_routing.h"
...
inline uint8_t xinputw_physical_source_count() {
  return min((uint8_t)XINPUT_MULTI_CONTROLLERS, (uint8_t)MAX_USB_OUT);
}

inline void service_xinputw_slot_routing() {
  if (get_effective_output_mode() != OUTPUT_XINPUTW) {
    return;
  }

  const uint8_t sourceCount =
    xinputw_physical_source_count();
  uint32_t connectedMask = 0;
  for (uint8_t source = 0; source < sourceCount; ++source) {
    if (controllerFrameConst(source).connected) {
      connectedMask |= 1UL << source;
    }
  }

  if (!xinputw_slot_routing_update(sourceCount, connectedMask, true)) {
    return;
  }

  // A physical port may move to a lower receiver child after a disconnect.
  // Clear feedback tied to the old route and force the surviving state into
  // its new child immediately, even if its buttons have not changed.
  for (uint8_t source = 0; source < sourceCount; ++source) {
    rumble_callback(source, 0, 0);
    if (controllerFrameConst(source).connected) {
      requestControllerFrameDelivery(source);
    }
  }
}
