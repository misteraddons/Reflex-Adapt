...
uint16_t webhid_get_settings_report(uint8_t* buffer) {
  ...
  buffer[WEBHID_SETTINGS_HOTKEY_FLAGS_INDEX] =
      ...
      ((menu_mister_output << WEBHID_SETTINGS_MISTER_OUTPUT_SHIFT) & WEBHID_SETTINGS_MISTER_OUTPUT_MASK);
  buffer[45] = 0;  // Retired 1P input/output view; single pads are always centered.
...
void webhid_write_settings_report(const uint8_t* buffer, uint16_t bufsize) {
  ...
  //             49=MiSTer/Linux output preference,
  ...
  switch (setting_id) {
    case 0:
      ...
      configuredOutputMode = sanitizeConfiguredOutputMode(
          (outputMode_t)loadSettingValue(SettingId::ConfiguredOutputMode, deviceMode));
    ...
    case 35:
      ...
      break;
    case 36:
      break;  // Retired 1P input/output view.
    case 37:
      if (value > 6) value = 1;
    ...
    case WEBHID_SETTING_MISTER_OUTPUT:
      if (value > HOST_OUTPUT_PREFERENCE_KEYBOARD) value = HOST_OUTPUT_PREFERENCE_DINPUT;
      {
        const bool misterOutputDescriptorChanged =
          value != menu_mister_output &&
          configuredOutputMode == OUTPUT_AUTO &&
          autoDetectState == AUTO_STATE_FALLBACK_HID;
        if (value != menu_mister_output) {
          auto_detect_clear_scratch_state();
        }
        saveSystemSettingByte(SettingId::MisterOutput, value);
        menu_mister_output = value;
    ...
    case 0xFF: {
      ...
      outputMode_t newOutputMode = sanitizeConfiguredOutputMode((outputMode_t)value);
