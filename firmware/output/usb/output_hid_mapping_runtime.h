...
inline bool isPositionButtonMapActive() {
  // Switch software expects each supported controller family to expose one
  // canonical layout. Ignore a stale Position setting while Switch is active.
  return get_effective_output_mode() != OUTPUT_SWITCHPRO &&
         inputModeUsesButtonMapMode() &&
         !is_automatic_nso_profile_active() &&
         button_map_mode == 1;
}

inline bool isPositionButtonMapActiveForXInputOutput() {
#ifdef ENABLE_INPUT_N64
  // XInput only has four face buttons, so keep the N64's two named buttons
  // and place the four C buttons around them consistently: C-Down/A,
  // C-Right/B, C-Left/X, and C-Up/Y. DInput retains the user's Button Map
  // preference because it has enough distinct button bits for every control.
