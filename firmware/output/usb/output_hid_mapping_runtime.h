...
inline bool isPositionButtonMapActive() {
  return effectivePositionButtonMap(
    ...
    is_nso_special_active(),
    button_map_mode);
}

inline bool isPositionButtonMapActiveForXInputOutput() {
#ifdef ENABLE_INPUT_N64
  // XInput keeps the N64's named A/B buttons while C buttons use the right stick.
  if (deviceMode == RZORD_N64) {
    return false;
  }
#endif
#ifdef ENABLE_INPUT_VBOY
  // Virtual Boy follows the same policy: named A/B plus right-D-pad on R-stick.
  if (deviceMode == RZORD_VBOY) {
    return false;
  }
#endif
...
inline bool hid_analog_mouse_output_active(uint8_t port,
                                           outputMode_t outputMode) {
  if ((outputMode != OUTPUT_HID && outputMode != OUTPUT_MISTER) ||
      analog_mouse_mode == ANALOG_MOUSE_OFF || port >= MAX_USB_OUT) {
    return false;
  }
  const controller_state_t& frame = controllerFrameConst(port);
  if (!frame.connected) return false;
  return analog_mouse_mode == ANALOG_MOUSE_RIGHT
    ? frame.HAS_ANALOG_STICK_AUX
    : frame.HAS_ANALOG_STICK_MAIN;
}
constexpr int16_t kAnalogMouseDeadzone = 24;

inline int8_t hid_analog_mouse_delta(int16_t axis) {
  if (axis >= -kAnalogMouseDeadzone &&
      axis <= kAnalogMouseDeadzone) {
    return 0;
  }
  const uint8_t speed =
    (analog_mouse_speed >= 1 && analog_mouse_speed <= ANALOG_MOUSE_SPEED_MAX)
      ? analog_mouse_speed : ANALOG_MOUSE_SPEED_DEFAULT;
  int16_t delta = (axis * speed) / (16 * ANALOG_MOUSE_SPEED_UNITY);
  if (delta == 0) delta = axis < 0 ? -1 : 1;
  return static_cast<int8_t>(delta);
}

inline void __not_in_flash_func(map_hid_analog_mouse_output)(uint8_t port) {
  const controller_state_t& frame = controllerFrameConst(port);
  _hidmouse = _default_hidmouse;
  // Primary face buttons become left/right mouse buttons.
  if (frame.A) {
  ...
  const bool useRight = analog_mouse_mode == ANALOG_MOUSE_RIGHT;
  _hidmouse.x = hid_analog_mouse_delta(useRight ? frame.RX : frame.LX);
  _hidmouse.y = hid_analog_mouse_delta(useRight ? frame.RY : frame.LY);
}
...
inline void map_hidjogconmister_output(uint8_t port) {
  ...
  const bool spinnerMode = std::strcmp(frame.controller_type_name, "JogCon-S") == 0;
  const bool paddleMode = std::strcmp(frame.controller_type_name, "JogCon-P") == 0 ||
                          std::strcmp(frame.controller_type_name, "JogCon-W") == 0;
  _jogcon.spinner_axis = spinnerMode ? frame.spinner : 0;
  _jogcon.paddle_axis = paddleMode
                           ? convertAnalogPrecision(frame.LX, input_precision, output_precision) + analog_mid
                           : analog_mid;
}
