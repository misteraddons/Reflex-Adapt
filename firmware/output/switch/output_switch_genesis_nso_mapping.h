...
// Switch full-input report button bits used to compensate Genesis, digital
// Saturn, and six-button PCE while they retain the shared 057E:2009 Switch Pro
// USB identity. Opt-in Genesis NSO mode uses the native 057E:201E identity and
// direct report fields instead; the official capture is
...
template <typename Frame>
inline uint32_t pack_jaguar_six_button_position_bits(const Frame& frame) {
  uint32_t bits = 0;
  if (frame.X) bits |= kY;    // Jaguar C -> Genesis A.
  if (frame.A) bits |= kB;    // Jaguar B -> Genesis B.
  if (frame.B) bits |= kA;    // Jaguar A -> Genesis C.
  if (frame.L1) bits |= kL;   // Jaguar 7 -> Genesis X.
  if (frame.Y) bits |= kX;    // Jaguar 8 -> Genesis Y.
  if (frame.R1) bits |= kR;   // Jaguar 9 -> Genesis Z.
  return bits;
}

template <typename Frame, typename Report>
inline void apply_jaguar_six_button_position_bits(
    const Frame& frame, Report& report) {
  uint32_t bits =
      static_cast<uint32_t>(report.buttons[0]) |
      (static_cast<uint32_t>(report.buttons[1]) << 8) |
      (static_cast<uint32_t>(report.buttons[2]) << 16);
  bits = (bits & ~kGenesisSixButtonPositionMask) |
         pack_jaguar_six_button_position_bits(frame);
  report.buttons[0] = static_cast<uint8_t>(bits);
  report.buttons[1] = static_cast<uint8_t>(bits >> 8);
  report.buttons[2] = static_cast<uint8_t>(bits >> 16);
}
