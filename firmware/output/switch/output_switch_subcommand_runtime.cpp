...
void SwitchCommon::spi_read() {
  ...
  constexpr uint8_t kMaxSpiReadLength = sizeof(_report) - 21;
  if (read_length > kMaxSpiReadLength) {
    read_length = kMaxSpiReadLength;
  }
...
void SwitchCommon::enable_vibration() {
  ...
  _vibration_idx = 0;
  _vibration_report = _vibration_enabled ? VIB_OPTS[_vibration_idx] : 0;
}
