...
inline void map_switchpro_output(uint8_t port) {
  ...
  bool nes_native_layout_on_switch = false;
  bool snes_native_layout_on_switch = false;
  bool genesis_native_layout_on_switch = false;
  ...
  nes_native_layout_on_switch = (deviceMode == RZORD_NES);
#endif
#ifdef ENABLE_INPUT_SNES
  snes_native_layout_on_switch = (deviceMode == RZORD_SNES);
#endif
  const bool native_nso_face_layout =
    nes_native_layout_on_switch || snes_native_layout_on_switch ||
    genesis_native_layout_on_switch || saturn_digital_on_switch;
  ...
  if (virtual_boy_on_switch) {
    ...
    // Switch always preserves native GameCube labels.
  ...
  switchpro[port]->switchCommon->_switchReport.l =
    genesis_native_layout_on_switch
      ? frame.L1
      : (n64_nso_on_switch
          ? frame.L1
  ...
  } else if (genesis_native_layout_on_switch) {
  ...
  } else if (nes_native_layout_on_switch || snes_native_layout_on_switch) {
  ...
  if (genesis_native_layout_on_switch) {
