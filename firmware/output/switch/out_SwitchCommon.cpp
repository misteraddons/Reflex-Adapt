...
void SwitchCommon::setSwitchRequestReport(const uint8_t *report, int report_size) {
  set_empty_switch_request_report();
  if (!report || report_size <= 0) {
    return;
  }

  if (report_size > static_cast<int>(sizeof(_switchRequestReport))) {
    report_size = sizeof(_switchRequestReport);
  }
  memcpy(_switchRequestReport, report, report_size);
  _switchRequestReportSize = report_size;
}

uint8_t *SwitchCommon::generate_report() {
  set_empty_report();
  _report[0] = 0xa1;
  if (_switchRequestReportSize < 11 || _switchRequestReport[0] != 0x01) {
    set_full_input_report();
    return _report;
  }

  const uint8_t subcommand = _switchRequestReport[10];
  switch (subcommand) {
    ...
    case SPI_READ:
      set_subcommand_reply();
      if (_switchRequestReportSize >= 16) {
        spi_read();
      } else {
        set_unknown_subcommand(subcommand);
      }
      break;
    case SET_MODE:
      set_subcommand_reply();
    ...
    case TOGGLE_IMU:
      set_subcommand_reply();
      if (_switchRequestReportSize >= 12) {
        toggle_imu();
      } else {
        set_unknown_subcommand(subcommand);
      }
