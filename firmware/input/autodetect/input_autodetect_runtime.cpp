...
constexpr uint32_t kNoDataHostInputDisconnectDebounceMs = 1000;

DeviceEnum noDataHostResolvedMode = RZORD_NONE;
uint32_t noDataHostInputMissingSinceMs = 0;

bool deferNoDataHostResolvedDisconnect(bool noDataHostFallback,
                                       bool waitingForInitialResolve,
                                       bool inputConnected,
                                       uint32_t now) {
  if (!noDataHostFallback || waitingForInitialResolve) {
    noDataHostResolvedMode = RZORD_NONE;
    noDataHostInputMissingSinceMs = 0;
    return false;
  }

  if (noDataHostResolvedMode != deviceMode) {
    noDataHostResolvedMode = deviceMode;
    noDataHostInputMissingSinceMs = 0;
  }
  ...
  if (noDataHostInputMissingSinceMs == 0) {
    noDataHostInputMissingSinceMs = now == 0 ? 1 : now;
    return true;
  }

  return (uint32_t)(now - noDataHostInputMissingSinceMs) <
         kNoDataHostInputDisconnectDebounceMs;
}
...
bool runHotswapWiiQuickPass(AutoDetectResult (&portResults)[INPUT_MIXED_PORT_COUNT],
                             DeviceEnum (&portModes)[INPUT_MIXED_PORT_COUNT],
                             uint8_t& detected_port) {
  #ifdef ENABLE_INPUT_WII
  for (uint8_t port = 0; port < INPUT_MIXED_PORT_COUNT; ++port) {
    portResults[port] = detectAutoInputPortWiiOnly(port, true);
    portModes[port] = autoDetectResultToDeviceMode(portResults[port]);
    if (portModes[port] != RZORD_NONE) {
      autoDetectResult = portResults[port];
      detected_port = port;
...
bool runHotswapFastStrictBusPass(AutoDetectResult (&portResults)[INPUT_MIXED_PORT_COUNT],
  ...
  if (runHotswapWiiQuickPass(portResults, portModes, detected_port)) {
    return true;
  }
...
DeviceEnum runAutoDetectionJoybusOnly(bool is_hotswap) {
  autoDetectBenchmarkMark(ADBENCH_DETECT_START, is_hotswap ? 1 : 0);
  const uint32_t detect_started = millis();
  uint8_t detected_port = 0xFF;
  uint8_t timing_flags = is_hotswap ? INPUT_AUTODETECT_TIMING_HOTSWAP : 0;
  AutoDetectResult portResults[INPUT_MIXED_PORT_COUNT] = {
    AUTODETECT_NONE, AUTODETECT_NONE
  };
  DeviceEnum portModes[INPUT_MIXED_PORT_COUNT] = {
    RZORD_NONE, RZORD_NONE
  };
  ...
  clearAutoDetectResult();
  for (uint8_t port = 0; port < INPUT_MIXED_PORT_COUNT; ++port) {
    portResults[port] = detectAutoInputPortJoybusOnly(port, is_hotswap);
    portModes[port] = autoDetectResultToDeviceMode(portResults[port]);
    if (detected_port == 0xFF && portModes[port] != RZORD_NONE) {
      detected_port = port;
    }
  ...
  selectPrimaryAutoDetectResult(portResults, detected_port);
  configureInputMixedAutoDetectModes(portModes[0], portModes[1]);
  const DeviceEnum detectedMode = (portModes[0] != RZORD_NONE)
      ? portModes[0]
...
bool checkAutoDetectHotSwap() {
  ...
  // After the no-data-host fallback window, the normal home releases Auto Input;
  // any real USB host immediately restores exclusive output detection.
  const bool noDataHostFallback = auto_detect_no_data_host_fallback_active();
  ...
  const bool inputConnected =
      anyInputFrameConnected(MAX_USB_OUT) ||
      activeInputAdapterHasPhysicalConnectionForHotSwap();
  if (deferNoDataHostResolvedDisconnect(
        noDataHostFallback, waitingForInitialResolve, inputConnected, now)) {
    return false;
  }
