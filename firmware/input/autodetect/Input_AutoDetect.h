...
// 4. 3DO - strict stable serial signature
// 5. Saturn/Megadrive - TH multiplexing
// 6. PSX - SPI protocol on pin 13 (only if Joybus not detected)
// 7. Dreamcast - Maple Bus on pins 08-09
...
public:
  ...
  static AutoDetectResult detectPortWiiOnly(uint8_t port, bool is_hotswap = false);
  static AutoDetectResult detectPortShiftRegisterOnly(uint8_t port, bool is_hotswap = false);
  static AutoDetectResult detectPortJoybusOnly(uint8_t port, bool is_hotswap = false);
  static AutoDetectResult detectPortSaturnSharedBusOnly(uint8_t port, bool is_hotswap = false);
  #ifdef ENABLE_INPUT_3DO
  static uint8_t capture3doDiagnosticFrames(uint8_t port,
                                            uint16_t* frames,
                                            uint8_t frame_count,
                                            uint8_t* idle_level = nullptr);
  #endif
