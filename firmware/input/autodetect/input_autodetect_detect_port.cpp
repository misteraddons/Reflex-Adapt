...
AutoDetectResult AutoDetector::detectPortWiiOnly(uint8_t port, bool is_hotswap) {
  (void)is_hotswap;
  if (port >= kAutoDetectPortCount) {
    return AUTODETECT_NONE;
  }
...
AutoDetectResult AutoDetector::detectPortJoybusOnly(uint8_t port, bool is_hotswap) {
  if (port >= kAutoDetectPortCount) {
    return AUTODETECT_NONE;
  }
  ...
  const autodetect_pins_t& pins = autodetect_pins[port];
  return finish(probeJoybus(pins, port, is_hotswap));
  #else
  (void)is_hotswap;
  return finish(AUTODETECT_NONE);
  #endif
...
AutoDetectResult AutoDetector::detectPortSaturnSharedBusOnly(uint8_t port, bool is_hotswap) {
  if (port >= kAutoDetectPortCount) {
    return AUTODETECT_NONE;
  }
  ...
  // 3DO shares Saturn TH/TL/TR and must retain first refusal over Saturn.
  ...
  AutoDetectResult saturn = probeSaturn(pins, port, nullptr, is_hotswap);
...
AutoDetectResult AutoDetector::detectPort(uint8_t port, bool is_hotswap) {
  ...
  if (pce != AUTODETECT_NONE || pceConfirmed) {
    ...
    if (pceConfirmed) {
      ...
      if (pceSaturn == AUTODETECT_SATURN) {
        deferredSharedBusSaturn = pceSaturn;
      }
      #if defined(ENABLE_INPUT_SATURN) || defined(ENABLE_INPUT_MEGADRIVE)
      if (deferredSharedBusSaturn == AUTODETECT_NONE) {
        resetSharedPins(pins, is_hotswap);
        if (confirmSaturnViaSaturnLib(pins, port, is_hotswap)) {
          deferredSharedBusSaturn = AUTODETECT_SATURN;
        }
      }
      #endif
    }
  ...
  #ifdef ENABLE_INPUT_3DO
  // 3DO CLK/OUT/IN share Saturn TH/TL/TR. A connected 3DO pad can look like
  // a Saturn digital pad to SaturnLib, so qualify its stricter serial frame
  // before accepting either a deferred or primary Saturn result.
  #ifdef AUTODETECT_DEBUG
  ...
  if (deferredSharedBusSaturn != AUTODETECT_NONE) {
    return finish(deferredSharedBusSaturn);
  }
