...
uint8_t AutoDetector::capture3doDiagnosticFrames(uint8_t port,
                                                 uint16_t* frames,
                                                 uint8_t frame_count,
                                                 uint8_t* idle_level) {
  if (port >= kAutoDetectPortCount || frames == nullptr || frame_count == 0) {
    return 0;
  }
  ...
  // Preload idle-high before enabling the two output drivers. The diagnostic
  // is intentionally bounded and restores all three lines to inputs afterward.
  ...
  if (idle_level != nullptr) {
    *idle_level = gpio_get(pins.tdo_in) ? 1 : 0;
  }

  for (uint8_t pass = 0; pass < frame_count; ++pass) {
  ...
  gpio_set_dir(pins.tdo_in, GPIO_IN);
  gpio_pull_up(pins.tdo_clk);
  gpio_pull_up(pins.tdo_out);
  gpio_pull_up(pins.tdo_in);
  return frame_count;
}
...
AutoDetectResult AutoDetector::probe3DO(const autodetect_pins_t& pins, bool is_hotswap) {
  ...
  // Preload the idle levels before enabling the output drivers. 3DO pads need
  // an idle-high interval before the first transaction, just like manual mode.
  gpio_put(pins.tdo_clk, HIGH);
  gpio_put(pins.tdo_out, HIGH);
  gpio_set_dir(pins.tdo_clk, GPIO_OUT);
  gpio_set_dir(pins.tdo_out, GPIO_OUT);
