...
bool inputHotSwapNeedsUsbDescriptorReenumeration() {
  ...
  const bool automaticN64NsoProfileChanged =
      canonicalizeOutputMode(get_effective_output_mode()) == OUTPUT_SWITCHPRO &&
      input_mode_has_automatic_nso_profile(deviceMode) && nso_special &&
      expectedUsbSlots != max_devices;
  if (automaticN64NsoProfileChanged) return true;

  if (connectedPlayers <= max_devices &&
...
bool switchProfileHotSwapChangedUsbDescriptor(switchpro_mode_enum previousProfile) {
  return output_runtime_is_switchpro_mode() &&
         previousProfile != SwitchCommon::switchpro_mode;
}
...
bool autoHomeNeedsUsbDescriptorReenumeration() {
  const uint8_t expectedUsbSlots = bootUsbSlotCountForModeAndInput(
      ...
      connectedInputFrameCount(inputPortCount()));
  return expectedUsbSlots != max_devices;
}
...
void applyDetectedInputModeLive(DeviceEnum newMode, bool assistedMode) {
  ...
  // Keep profile selection in the transition owner as well as the individual
  // input modules. A newly resolved N64 input changes the USB VID/PID and must
  // take the descriptor-reenumeration path below.
...
bool tryPassiveHotSwapWithSharedBusArbitration(
    const PassiveAssistScanResult& passiveCandidate,
    uint32_t passiveElapsedMs) {
  if (passiveCandidate.result == AUTODETECT_JPC) {
    const uint32_t arbitrationStartedMs = millis();
    resetAutoDetectPins();
    delayWithButtonPolling(inputAutoDetectBootSettleMs());
    const AutoDetectResult activeResult = detectAutoInputPortSaturnSharedBusOnly(
        passiveCandidate.port, true);
    if (activeResult != AUTODETECT_NONE) {
      setAutoDetectResult(activeResult);
      const DeviceEnum activeMode = autoDetectResultToDeviceMode(activeResult);
      uint8_t timingFlags = INPUT_AUTODETECT_TIMING_HOTSWAP;
      if (activeMode != RZORD_NONE) {
        timingFlags |= INPUT_AUTODETECT_TIMING_VALID;
      }
      recordInputAutodetectTiming(
          passiveElapsedMs + (millis() - arbitrationStartedMs),
          passiveCandidate.port,
          timingFlags);
      resetIdleTimer();
      return tryHotSwapToDetectedMode(activeMode);
    }
  }

  const DeviceEnum passiveMode = applyPassiveAutoDetectResult(
      passiveCandidate.result,
      passiveElapsedMs,
      passiveCandidate.port,
...
bool handlePassiveAssistedHotSwapQuick() {
  ...
  if (passiveQuick.result != AUTODETECT_NONE) {
    return tryPassiveHotSwapWithSharedBusArbitration(passiveQuick, 0);
  }
...
bool handlePassiveAssistedHotSwapFallback() {
  ...
  if (passiveResult.result != AUTODETECT_NONE) {
    return tryPassiveHotSwapWithSharedBusArbitration(
...
void restoreAutoDetectHomeAfterDisconnect(DeviceEnum disconnectedMode) {
  ...
  const bool switchProfileDescriptorReset =
      switchProfileHotSwapChangedUsbDescriptor(switchProfileBeforeInputSetup);
  clearAutoDetectDisconnectedFrames();
  resetIdleTimer();
  forceMainDisplayRefresh();
  autoDetectBenchmarkMark(ADBENCH_MAIN_REFRESH);
  if (psxOutputDescriptorReset ||
      switchProfileDescriptorReset ||
      autoHomeNeedsUsbDescriptorReenumeration()) {
