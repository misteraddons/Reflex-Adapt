...
constexpr uint8_t kAutoInputResolveSourcePreUsbNso = 7;
...
void resolveAutomaticSwitchProfileInputAtBoot(bool autoOutputResolvedBoot) {
    ...
    (defined(ENABLE_INPUT_N64) || defined(ENABLE_INPUT_GAMECUBE) || defined(ENABLE_INPUT_GBA))
  if (savedDeviceMode != RZORD_AUTODETECT ||
      deviceMode != RZORD_AUTODETECT) {
    return;
  }

  const bool resolvedSwitchProfileBoot =
      autoOutputResolvedBoot && output_runtime_is_switchpro_mode();
  const bool resolvedMisterIdentityBoot =
  ...
  // USB is still disconnected here. Resolve only the narrow Joybus family so
  // Switch can choose its native NSO profile and MiSTer can use the input-mode
  // serial identity (for example ReflexN64) on its first relevant enumeration.
  const DeviceEnum detectedMode = runAutoDetectionJoybusOnly(true);
  if (detectedMode == RZORD_NONE ||
      (resolvedSwitchProfileBoot &&
       !input_mode_has_automatic_nso_profile(detectedMode))) {
  ...
  recordInputAutoResolve(
      ...
      kAutoInputResolveSourcePreUsbNso, input_autodetect_last_port);
  setAutoInputResolvedGraceUntil(millis() + autoInputResolvedGraceMs(detectedMode));
  markInputHotSwapConnected();
#else
  (void)autoOutputResolvedBoot;
#endif
}
