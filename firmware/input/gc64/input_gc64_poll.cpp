...
bool __not_in_flash_func(RZInputGC64::poll)() {
  ...
  for (uint8_t port = 0; port < input_ports; ++port) {
    ...
    const JoybusDeviceType_Enum observed_type = joybus[port]->deviceType();
    if (observed_type != JOYBUS_DEVICE_NONE) {
      slotLastSeenMs[port] = now_ms;
    }
    const bool transient_disconnect =
        observed_type == JOYBUS_DEVICE_NONE &&
        dtype[port] != JOYBUS_DEVICE_NONE &&
        slotLastSeenMs[port] != 0 &&
        (uint32_t)(now_ms - slotLastSeenMs[port]) < SLOT_DISCONNECT_DEBOUNCE_MS;
    if (transient_disconnect) {
      // One failed Joybus transaction must not drop a live N64/GC frame. Keep
      // polling at the connected cadence so the next successful read recovers
      // before AUTO input hot-swap can fall back to scanning.
      continue;
    }
    const bool confirmed_disconnect =
        observed_type == JOYBUS_DEVICE_NONE && dtype[port] != JOYBUS_DEVICE_NONE;
    ...
    if (joybus[port]->stateChanged() || confirmed_disconnect) {
      resetState(i);
      if (joybus[port]->deviceJustChanged() || confirmed_disconnect) {
        JoybusDeviceType_Enum observedType = observed_type;
        ...
        dtype[port] = (wavebirdReceiverSeen[port] && observedType == JOYBUS_DEVICE_GCPAD)
                        ...
                        : observedType;
        if (dtype[port] == JOYBUS_DEVICE_NONE) {
          slotLastSeenMs[port] = 0;
        }
