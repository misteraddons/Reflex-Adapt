...
bool __not_in_flash_func(wii_extension_frame_is_corrupt)(
    NintendoExtensionCtrl::ExtensionController* controller) {
  ...
  // Wii extensions do not agree on reliable reserved constant bits. Accept
  // otherwise plausible frames so valid controllers do not fall back to AUTO.
  return false;
}
...
bool RZInputWii::wii_connect(uint8_t i) {
  ...
  uint8_t handoffPair = INPUT_WII_PIN_PAIR_INVALID;
  uint8_t handoffIdentity[6] = {};
  const bool haveHandoff =
      peekWiiAutodetectIdentity(i, &handoffPair, handoffIdentity) &&
      handoffPair < INPUT_WII_PIN_PAIR_COUNT;

  auto tryPair = [&](uint8_t pair, const uint8_t* identity) -> bool {
    if (!beginWiiBus(i, pair)) {
      return false;
    }
    const bool result = identity != nullptr
        ? wii[i]->connectFromIdentity(identity)
        : wii[i]->connect();
  ...
  if (haveHandoff) {
    if (tryPair(handoffPair, handoffIdentity)) {
      clearWiiAutodetectIdentity(i);
      handoffFailCount[i] = 0;
      return true;
    }
    if (++handoffFailCount[i] < 10) {
      return false;
    }
    clearWiiAutodetectIdentity(i);
    handoffFailCount[i] = 0;
  ...
  if (activePinPair[i] < INPUT_WII_PIN_PAIR_COUNT &&
      (!haveHandoff || activePinPair[i] != handoffPair) &&
      tryPair(activePinPair[i], nullptr)) {
    return true;
  }
  ...
  for (uint32_t i = 0; i < input_ports; ++i) {
    ...
    if (!haveController[i]) {
      ...
      if (connected) {
        ...
        publishedController[i] = false;
        connectedAtUs[i] = nowUs;
      ...
      bool corruptFrame =
          updateOk && wii_extension_frame_is_corrupt(wii[i]);
      if (!updateOk || corruptFrame) {
        const bool retryOk = wii_update(i);
        if (retryOk) {
          const ExtensionType retryType = wii[i]->getControllerType();
          const bool retryCorruptFrame = wii_extension_frame_is_corrupt(wii[i]);
          if (!retryCorruptFrame) {
            updateOk = true;
            conType = retryType;
            corruptFrame = false;
          }
      ...
      if (traceEnabled) {
        wiiTraceRecord(WII_TRACE_EVENT_UPDATE, i, updateTraceStartUs, micros(),
                       (updateOk && !corruptFrame) ? 1 : 0,
                       updateFailCount[i], (uint8_t)conType);
      }
      if (!updateOk || corruptFrame) {
        settlingAfterBadFrame[i] = true;
        if ((uint32_t)(time_us_32() - connectedAtUs[i]) <
            WII_CONNECT_SETTLE_GRACE_US) {
          continue;
        }
        ...
        if (conType != ExtensionType::NoController) {
          publishedController[i] = true;
