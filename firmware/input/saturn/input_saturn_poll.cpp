...
SatDeviceType_Enum RZInputSaturn::stabilizeMegadriveType(
  ...
  if (detectedType == SAT_DEVICE_MEGA3) {
    if (stableMegadriveType[slot] == SAT_DEVICE_MEGA6) {
      // Keep the confirmed subtype until physical-disconnect cleanup clears
      // this slot. Probe misses and button activity must not demote it.
      return SAT_DEVICE_MEGA6;
    }
...
bool __not_in_flash_func(RZInputSaturn::poll)() {
  if (lastMegadriveTransactionFinishedUs != 0) {
    bool connectedMegadrive = false;
    for (uint8_t i = 0; i < MAX_USB_OUT; ++i) {
      if (isMegadriveType(dtype[i])) {
        connectedMegadrive = true;
        break;
      }
    }
    if (connectedMegadrive &&
        (uint32_t)(micros() - lastMegadriveTransactionFinishedUs) <
            MEGADRIVE_MIN_TH_HIGH_US) {
      // The shared scheduler can catch up after a late loop. Never let that
      // shorten the controller's required TH-high recovery period.
      return false;
    }
  }
  ...
  for (uint8_t port = 0; port < input_ports; ++port) {
    ...
    for (uint8_t c = 0; c < activeSlots; ++c) {
      ...
      SatDeviceType_Enum detectedType = stabilizeMegadriveType(i, rawDetectedType);
      ...
      if (typeChanged && !isMegadriveType(detectedType)) {
        // A positively identified non-Genesis device is a replacement, not a
        // transient Mega6 probe miss.
        m30Identity[i].reset();
      }
      const bool m30WasIdentified = m30Identity[i].identified();
      m30Identity[i].observe(
          detectedType == SAT_DEVICE_MEGA6,
          sc.digitalPressed(SAT_M30_HOME),
          sc.digitalPressed(SAT_M30_STAR));
      const bool m30JustIdentified =
          !m30WasIdentified && m30Identity[i].identified();
  ...
  bool found_fast_saturn = false;
  bool found_connected_megadrive = false;
  for (uint8_t i = 0; i < MAX_USB_OUT; ++i) {
    switch (dtype[i]) {
      case SAT_DEVICE_NONE:
      case SAT_DEVICE_NOTSUPPORTED:
  ...
  lastMegadriveTransactionFinishedUs =
      found_connected_megadrive ? micros() : 0;
