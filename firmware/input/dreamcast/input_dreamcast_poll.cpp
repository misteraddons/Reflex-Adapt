...
bool RZInputDreamcast::poll() {
  ...
  for (uint8_t i = 0; i < input_ports && i < MAX_USB_OUT; ++i) {
...

    {
      ...
      const MapleDeviceInfo& deviceInfo = maple[i]->getDeviceInfo();

      resetState(i);

      frame.PAD_U = maple[i]->buttonPressed(DC_BTN_UP);
      frame.PAD_D = maple[i]->buttonPressed(DC_BTN_DOWN);
      const bool isWheelController = dreamcastIsWheelController(deviceInfo);
      ...
      const bool hasLeftTrigger =
        dreamcastHasControllerCapability(deviceInfo, kDreamcastCapLTrigger);
      const bool hasRightTrigger =
        dreamcastHasControllerCapability(deviceInfo, kDreamcastCapRTrigger);
      frame.ANALOG_L2 = hasLeftTrigger ? dc.ltrigger : 0;
      frame.ANALOG_R2 = hasRightTrigger ? dc.rtrigger : 0;
      frame.HAS_ANALOG_TRIGGERS = hasLeftTrigger || hasRightTrigger;

      frame.L2 = hasLeftTrigger && dc.ltrigger > 128;
      frame.R2 = hasRightTrigger && dc.rtrigger > 128;

      constexpr DeviceEnum analogMode = RZORD_DREAMCAST;
      const bool hasMainX =
        dreamcastHasControllerCapability(deviceInfo, kDreamcastCapAnalogX);
      const bool hasMainY =
        dreamcastHasControllerCapability(deviceInfo, kDreamcastCapAnalogY);
      frame.HAS_ANALOG_STICK_MAIN = hasMainX || hasMainY;
      frame.LX = 0;
      frame.LY = 0;
      if (hasMainX) {
        const int16_t raw_lx = dreamcastAxisFromRaw(dc.joyx);
        frame.LX = wii_analog_range == CLASSIC_ANALOG_RANGE_LEARN
                     ? applyClassicAnalogLearnAxis(analogMode, i, CLASSIC_ANALOG_AXIS_LX, raw_lx)
                     : raw_lx;
        recordClassicAnalogRangeAxis(analogMode, i, CLASSIC_ANALOG_AXIS_LX,
                                     raw_lx, frame.LX);
      }
      if (hasMainY) {
        const int16_t raw_ly = dreamcastAxisFromRaw(dc.joyy);
        frame.LY = wii_analog_range == CLASSIC_ANALOG_RANGE_LEARN
                     ? applyClassicAnalogLearnAxis(analogMode, i, CLASSIC_ANALOG_AXIS_LY, raw_ly)
                     : raw_ly;
        recordClassicAnalogRangeAxis(analogMode, i, CLASSIC_ANALOG_AXIS_LY,
                                     raw_ly, frame.LY);
      }
      ...
      const bool hasAuxX =
        dreamcastHasControllerCapability(deviceInfo, kDreamcastCapAnalog2X);
      const bool hasAuxY =
        dreamcastHasControllerCapability(deviceInfo, kDreamcastCapAnalog2Y);
      frame.RX = 0;
      frame.RY = 0;
      if (hasAuxX) {
