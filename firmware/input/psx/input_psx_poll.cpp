...
bool psxProtocolHasDualSticks(PsxControllerProtocol protocol) {
...
void RZInputPSX::mapMultiController(uint8_t i, PsxControllerData& cont) {
  ...
  if (cont.protocol != PSPROTO_DIGITAL) {
    hasLeftStick = cont.getLeftAnalog(lx, ly);
    hasRightStick = cont.getRightAnalog(rx, ry);
  }
  const bool hasDualSticks = psxProtocolHasDualSticks(cont.protocol);
  frame.HAS_ANALOG_STICK_MAIN = hasDualSticks && hasLeftStick;
  frame.HAS_ANALOG_STICK_AUX = hasDualSticks && hasRightStick;
  ...
  for (uint8_t port = 0; port < input_ports; ++port) {
    ...
    if (!haveController[i]) {
      ...
      if (!isWaiwai) {
        ...
        bool rumble_configured =
          rumbleConfiguredProto[i] == PSPROTO_DUALSHOCK ||
          rumbleConfiguredProto[i] == PSPROTO_DUALSHOCK2;
        ...
        // Preserve the host's independent XInput motor channels. The PSX
        // right/small motor is binary; the left/large motor is variable.
        if (rumble_configured) {
          const uint8_t physical_left =
            effective_left != 0 && effective_left < kPsxLargeMotorMinimum
              ? kPsxLargeMotorMinimum
              : effective_left;
          // Keep the configured motor-byte mapping stable and transmit an
          // explicit 0,0 on every idle poll. Reconfiguring the controller at
          // effect boundaries can leave a DualShock 2 motor latched.
      ...
      if (haveController[i] && isReadSuccess) {
        ...
        if (isNeGcon) {
          ...
          byte rx = 0x80;
          byte ry = 0x80;
          const bool hasLeftStick = psx[i]->getLeftAnalog(lx, ly);
          const bool hasRightStick = psx[i]->getRightAnalog(rx, ry);
          const bool specialControllerWithoutSticks =
            specialDpadMask[i] == SPECIALMASK_POPN ||
            specialDpadMask[i] == SPECIALMASK_JET;
          const bool hasDualSticks = psxProtocolHasDualSticks(proto);
          frame.HAS_ANALOG_STICK_MAIN =
            hasDualSticks && hasLeftStick && !specialControllerWithoutSticks;
          frame.HAS_ANALOG_STICK_AUX =
            hasDualSticks && hasRightStick && !specialControllerWithoutSticks;
          ...
          if (specialControllerWithoutSticks) {
            frame.LX = 0;
            frame.LY = 0;
            frame.RX = 0;
            frame.RY = 0;
