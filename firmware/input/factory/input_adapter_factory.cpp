...
void createActiveInputAdapter() {
  ...
  // A concrete family driver must keep polling every Classic2USB connector.
  // The detected port identifies the primary AUTO result and hot-swap probe;
  // it is not a permanent poll mask. Restricting the driver here left the
  // second reserved USB player neutral even when a matching controller was
  // connected. RZInputMixed applies explicit per-port masks when the mixed
