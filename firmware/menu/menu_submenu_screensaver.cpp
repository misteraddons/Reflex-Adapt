...
struct ScreensaverRenderSnapshot {
  bool valid;
  uint8_t cursor;
  uint8_t mode;
  uint8_t animation;
...
ScreensaverRenderSnapshot screensaver_render_snapshot{};
bool screensaver_submenu_initialized = false;

bool screensaverRenderSnapshotMatches(uint8_t cursor, uint8_t mode,
                                      uint8_t animation) {
  return screensaver_render_snapshot.valid &&
         screensaver_render_snapshot.cursor == cursor &&
         screensaver_render_snapshot.mode == mode &&
         screensaver_render_snapshot.animation == animation;
}

void captureScreensaverRenderSnapshot(uint8_t cursor, uint8_t mode,
                                      uint8_t animation) {
  screensaver_render_snapshot = {true, cursor, mode, animation};
}

void invalidateScreensaverRenderSnapshot() {
  screensaver_render_snapshot.valid = false;
}
...
bool handleScreensaverSubmenu(
  ...
  if (!screensaver_submenu_initialized) {
    ...
    invalidateScreensaverRenderSnapshot();
    display.clear();
  }

  if (!screensaverRenderSnapshotMatches(
          screensaver_submenu_cursor, menu_screensaver,
          menu_screensaver_anim)) {
  ...
  for (uint8_t row = 0; row < visible_rows; row++) {
    ...
    captureScreensaverRenderSnapshot(
        screensaver_submenu_cursor, menu_screensaver,
        menu_screensaver_anim);
  }
