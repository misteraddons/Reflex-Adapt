...
uint8_t rumbleLevelToByte(uint8_t level) {
  return level == RUMBLE_MODE_OFF ? 0 : 255;
}
}  // namespace
...
void QuickConfigMenu::cycleMainMenuItemNext() {
  ...
  switch (item) {
    ...
    case QCI_GC_L_SWITCH:
      temp_gamecube_l_switch =
          temp_gamecube_l_switch == GAMECUBE_L_SWITCH_L
            ? GAMECUBE_L_SWITCH_ZL
            : GAMECUBE_L_SWITCH_L;
      break;
    case QCI_ANALOG_MOUSE_MODE:
      temp_analog_mouse_mode = static_cast<uint8_t>(
        (temp_analog_mouse_mode + 1) %
        (menuModeCurrentControllerHasRightStick(temp_input_mode) ? 3 : 2));
      break;

    case QCI_ANALOG_MOUSE_SPEED:
      temp_analog_mouse_speed = temp_analog_mouse_speed >= ANALOG_MOUSE_SPEED_MAX
        ? 1 : static_cast<uint8_t>(temp_analog_mouse_speed + 1);
      break;
    ...
    case QCI_JOGCON_MODE:
      temp_jogcon_mode = (temp_jogcon_mode + 1) % 4;
      rebuildVisibleAndClampSelection();
      break;

    case QCI_JOGCON_FORCE:
      temp_jogcon_force = temp_jogcon_force == 1 ? 3 :
                          temp_jogcon_force == 3 ? 7 :
                          temp_jogcon_force == 7 ? 15 : 1;
      break;
...
void QuickConfigMenu::cycleMainMenuItemPrev() {
  ...
  switch (item) {
    ...
    case QCI_ANALOG_MOUSE_MODE:
      if (temp_analog_mouse_mode == ANALOG_MOUSE_OFF) {
        temp_analog_mouse_mode = menuModeCurrentControllerHasRightStick(temp_input_mode)
          ? ANALOG_MOUSE_RIGHT : ANALOG_MOUSE_LEFT;
      } else {
        --temp_analog_mouse_mode;
      }
      break;

    case QCI_ANALOG_MOUSE_SPEED:
      temp_analog_mouse_speed = temp_analog_mouse_speed <= 1
        ? ANALOG_MOUSE_SPEED_MAX
        : static_cast<uint8_t>(temp_analog_mouse_speed - 1);
      break;
    ...
    case QCI_JOGCON_MODE:
      temp_jogcon_mode = (temp_jogcon_mode == 0) ? 3 : temp_jogcon_mode - 1;
      rebuildVisibleAndClampSelection();
      break;

    case QCI_JOGCON_FORCE:
      temp_jogcon_force = temp_jogcon_force == 15 ? 7 :
                          temp_jogcon_force == 7 ? 3 :
                          temp_jogcon_force == 3 ? 1 : 15;
      break;
...
void QuickConfigMenu::select() {
  ...
  switch (state) {
    ...
    case QC_CONFIRM_NSO_1P:
      if (nso_1p_confirm_index == 1) {
        applyAndClose();
      } else {
        state = QC_MAIN_MENU;
      }
...
bool QuickConfigMenu::saveOrConfirmNsoSinglePlayer() {
  const outputMode_t selectedOutput =
    (temp_output_mode == OUTPUT_AUTO) ? get_effective_output_mode() : temp_output_mode;
  const bool enablingNativeNso =
    selectedOutput == OUTPUT_SWITCHPRO &&
    input_mode_supports_nso_special(temp_input_mode) &&
    temp_nso_special != 0 &&
    !menu_nso_special;
  if (enablingNativeNso) {
    state = QC_CONFIRM_NSO_1P;
    nso_1p_confirm_index = 0;
    buzzer.playMenuNav();
    return false;
  }
  applyAndClose();
...
void QuickConfigMenu::handleAnalogSelect() {
  ...
  switch (item) {
    ...
    case QCI_ANALOG_MOUSE_SPEED:
      temp_analog_mouse_speed =
        temp_analog_mouse_speed >= ANALOG_MOUSE_SPEED_MAX
          ? 1 : static_cast<uint8_t>(temp_analog_mouse_speed + 1);
      break;
...
void QuickConfigMenu::handleAnalogSelectBack() {
  ...
  switch (item) {
    ...
    case QCI_ANALOG_MOUSE_MODE:
      if (temp_analog_mouse_mode == ANALOG_MOUSE_OFF) {
        temp_analog_mouse_mode =
          menuModeCurrentControllerHasRightStick(temp_input_mode)
            ? ANALOG_MOUSE_RIGHT : ANALOG_MOUSE_LEFT;
      } else {
        --temp_analog_mouse_mode;
      }
...
void QuickConfigMenu::handleRumbleSelect(bool forward) {
  switch ((RumbleMenuItem)rumble_index) {
    ...
    case QCI_RUMBLE_RAMP_HEAVY:
      startHeavyRumbleRamp();
      break;
