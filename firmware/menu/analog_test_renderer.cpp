...
constexpr int16_t kAnalogTraceRadius = 25;
...
void drawAnalogTracePlot(const AnalogStickTrace& trace, bool octagonal,
  ...
  for (uint8_t i = 0; i < count; ++i) {
    ...
    const AnalogTraceDirection nextDirection =
      AnalogStickTrace::orderedDirection(
        octagonal, (uint8_t)((i + 1) % count));
    const AnalogTracePoint& point = trace.point(direction);
    const AnalogTracePoint& nextPoint = trace.point(nextDirection);
    if (point.valid && nextPoint.valid) {
      u8g2.drawLine(
        analogTracePlotCoordinate(kAnalogTraceCenterX, point.x, fullScale),
        analogTracePlotCoordinate(kAnalogTraceCenterY, point.y, fullScale),
        analogTracePlotCoordinate(kAnalogTraceCenterX, nextPoint.x, fullScale),
        analogTracePlotCoordinate(kAnalogTraceCenterY, nextPoint.y, fullScale));
    }
    if (point.valid) {
      u8g2.drawPixel(
        analogTracePlotCoordinate(kAnalogTraceCenterX, point.x, fullScale),
