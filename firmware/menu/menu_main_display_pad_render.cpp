...
constexpr uint8_t kNeGconWheelGaugeCol = (5 * kPadGlyphWidth) + 3;
constexpr uint8_t kNeGconWheelGaugeWidth = kPadGlyphWidth;
constexpr uint8_t kNeGconWheelGaugeRows = 4;
constexpr uint8_t kNeGconWheelGaugeHeight = kNeGconWheelGaugeRows * 8;
constexpr uint8_t kNeGconWheelGaugeCenterTop = (kNeGconWheelGaugeHeight / 2) - 1;
constexpr uint8_t kNeGconWheelGaugeCenterBottom = kNeGconWheelGaugeHeight / 2;
...
void drawNeGconWheelGauge(const controller_state_t* frame,
                          uint8_t colOffset,
                          uint8_t rowOffset) {
  int16_t twist = frame != nullptr ? frame->LX : 0;
  if (twist < -128) twist = -128;
  if (twist > 127) twist = 127;
  const uint16_t absTwist = twist < 0 ? (uint16_t)(-twist) : (uint16_t)twist;
  constexpr uint8_t halfSpan = kNeGconWheelGaugeCenterTop - 1;
  uint8_t magnitude = (uint8_t)((absTwist * halfSpan + 63) / 128);
  if (magnitude > halfSpan) magnitude = halfSpan;

  for (uint8_t page = 0; page < kNeGconWheelGaugeRows; ++page) {
    display.setCursor(colOffset + kNeGconWheelGaugeCol, rowOffset + page);
    for (uint8_t x = 0; x < kNeGconWheelGaugeWidth; ++x) {
      uint8_t column = 0;
      for (uint8_t bit = 0; bit < 8; ++bit) {
        const uint8_t y = page * 8 + bit;
        const bool outline =
          x == 0 || x == kNeGconWheelGaugeWidth - 1 ||
          y == 0 || y == kNeGconWheelGaugeHeight - 1 ||
          y == kNeGconWheelGaugeCenterTop ||
          y == kNeGconWheelGaugeCenterBottom;
        const bool fillUp =
          twist < 0 &&
          y < kNeGconWheelGaugeCenterTop &&
          y >= kNeGconWheelGaugeCenterTop - magnitude;
        const bool fillDown =
          twist > 0 &&
          y > kNeGconWheelGaugeCenterBottom &&
          y <= kNeGconWheelGaugeCenterBottom + magnitude;
        if (outline || ((x > 0 && x < kNeGconWheelGaugeWidth - 1) &&
                        (fillUp || fillDown))) {
...
void updateRealtimeButtons() {
  ...
  if (primaryNeGconTwistChanged) {
    drawNeGconWheelGauge(primaryAnalogFrame, primaryColOffset, kHomePadRowOffset);
  }
  ...
  if (showSecondPad && secondaryNeGconTwistChanged) {
    drawNeGconWheelGauge(secondaryAnalogFrame, kSecondPadColOffset, kHomePadRowOffset);
  }
