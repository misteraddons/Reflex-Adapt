...
constexpr uint8_t kHomeAnalogGaugeWidth =
  kSecondPadColOffset - (2 * kPadGlyphWidth);
constexpr uint8_t kHomeAnalogPadColOffset =
  kHomeAnalogGaugeWidth + kPadGlyphWidth;
constexpr uint8_t kHomeAnalogGaugeHeight = 32;
constexpr uint8_t kHomeAnalogGaugePages = kHomeAnalogGaugeHeight / 8;
constexpr int8_t kHomeAnalogGaugeCenterX = 30;
constexpr int8_t kHomeAnalogGaugeCenterY = 15;
constexpr int8_t kHomeAnalogGaugeRadius = 14;
constexpr int8_t kHomeAnalogGaugeDotRadius = 1;
...
constexpr uint8_t kNeGconAnalogGaugeGap = kPadGlyphWidth / 2;
constexpr uint8_t kNeGconTwistGaugeRow = 7;
constexpr uint8_t kNeGconTwistGaugeWidth = 60;
...
enum class HomeAnalogGaugeKind : uint8_t {
  None = 0,
  Gate,
  DualAxes,
};

struct HomeAnalogGaugeState {
  bool initialized = false;
  DeviceEnum mode = RZORD_NONE;
  HomeAnalogGaugeKind kind = HomeAnalogGaugeKind::None;
  bool octagonal = false;
  int16_t fullScale = 0;
  int8_t dotX = kHomeAnalogGaugeCenterX;
  int8_t dotY = kHomeAnalogGaugeCenterY;
  int16_t axes[4] = { 0, 0, 0, 0 };
};

HomeAnalogGaugeState homeAnalogGaugeState;

void drawCenterOutSignedAxisGauge(int16_t delta,
                                  uint8_t col,
                                  uint8_t row,
                                  uint8_t panelWidth);

uint8_t connectedHomeControllerCount() {
  uint8_t connected = 0;
...
HomeAnalogGaugeKind homeAnalogGaugeKindForFrame(
    DeviceEnum mode, const controller_state_t& frame) {
  if (!frame.connected || !frame.HAS_ANALOG_STICK_MAIN ||
      analogDiagnosticKindForFrame(mode, frame) != AnalogDiagnosticKind::Stick) {
    return HomeAnalogGaugeKind::None;
  }
  ...
  if (mode == RZORD_PSX) {
    return frame.HAS_ANALOG_STICK_AUX
      ...
      : HomeAnalogGaugeKind::None;
  }
#endif
#ifdef ENABLE_INPUT_WII
  if (mode == RZORD_WII) {
    if (std::strcmp(frame.controller_type_name, "Nunchuk") == 0) {
      return HomeAnalogGaugeKind::Gate;
    }
    const bool classic =
      std::strcmp(frame.controller_type_name, "Classic") == 0 ||
      std::strcmp(frame.controller_type_name, "ClassicPro") == 0;
    return classic && frame.HAS_ANALOG_STICK_AUX
      ? HomeAnalogGaugeKind::DualAxes
      : HomeAnalogGaugeKind::None;
  }
#endif
...
bool shouldShowHomeAnalogGauge(bool centeredSinglePad) {
  return centeredSinglePad &&
         connectedHomeControllerCount() == 1 &&
         homeAnalogGaugeKindForFrame(deviceMode, controllerFrameConst(0)) !=
           HomeAnalogGaugeKind::None;
}

int16_t homeAnalogGaugeFullScale(const controller_state_t& frame) {
#ifdef ENABLE_INPUT_N64
  if (deviceMode == RZORD_N64) {
    return kHomeN64AnalogFullScale;
  }
#endif
  return analogTraceFullScale(frame.sticks_precision_bits);
}

int8_t homeAnalogGaugeCoordinate(int8_t center, int16_t value,
                                 int16_t fullScale) {
  if (fullScale <= 0) {
    return center;
  }
  int32_t clamped = value;
  if (clamped < -fullScale) clamped = -fullScale;
  if (clamped > fullScale) clamped = fullScale;
  return (int8_t)(center +
    (clamped * kHomeAnalogGaugeRadius) / fullScale);
}

bool homeAnalogGaugeBackgroundPixel(uint8_t x, uint8_t y, bool octagonal) {
  const int16_t dx = (int16_t)x - kHomeAnalogGaugeCenterX;
  const int16_t dy = (int16_t)y - kHomeAnalogGaugeCenterY;
  ...
  if (!octagonal) {
    ...
    const int16_t radiusSquared =
      kHomeAnalogGaugeRadius * kHomeAnalogGaugeRadius;
    if (crosshair && distanceSquared <= radiusSquared) {
      return true;
    }
    const int16_t error = distanceSquared > radiusSquared
      ? distanceSquared - radiusSquared
      : radiusSquared - distanceSquared;
    return error <= kHomeAnalogGaugeRadius;
  }

  // Point-up octagon: cardinal directions end in vertices while diagonal
  // directions form the shoulders of the physical gate.
  constexpr int16_t shoulder = 10;
  constexpr int16_t corner = kHomeAnalogGaugeRadius - shoulder;
  const bool insideGate =
    ax <= kHomeAnalogGaugeRadius &&
    ay <= kHomeAnalogGaugeRadius &&
    corner * ax + shoulder * ay <=
      shoulder * kHomeAnalogGaugeRadius &&
    shoulder * ax + corner * ay <=
      shoulder * kHomeAnalogGaugeRadius;
  if (crosshair && insideGate) {
    return true;
  }
  const int16_t target = shoulder * kHomeAnalogGaugeRadius;
  const int16_t edgeA = corner * ax + shoulder * ay;
  const int16_t edgeB = shoulder * ax + corner * ay;
  constexpr int16_t edgeTolerance = 5;
  const int16_t errorA = edgeA > target ? edgeA - target : target - edgeA;
  const int16_t errorB = edgeB > target ? edgeB - target : target - edgeB;
  return (errorA <= edgeTolerance && edgeB <= target + edgeTolerance) ||
         (errorB <= edgeTolerance && edgeA <= target + edgeTolerance);
}

uint8_t homeAnalogGaugeColumnByte(uint8_t x, uint8_t page,
                                  int8_t dotX, int8_t dotY,
                                  bool octagonal) {
  uint8_t column = 0;
  const uint8_t firstY = page * 8;
  for (uint8_t bit = 0; bit < 8; ++bit) {
    const uint8_t y = firstY + bit;
    const int16_t dotDx = (int16_t)x - dotX;
    const int16_t dotDy = (int16_t)y - dotY;
    const bool dot =
      dotDx >= -kHomeAnalogGaugeDotRadius &&
      dotDx <= kHomeAnalogGaugeDotRadius &&
      dotDy >= -kHomeAnalogGaugeDotRadius &&
      dotDy <= kHomeAnalogGaugeDotRadius;
    if (dot || homeAnalogGaugeBackgroundPixel(x, y, octagonal)) {
...
void writeHomeAnalogGaugeSpan(uint8_t firstCol, uint8_t lastCol,
                              int8_t dotX, int8_t dotY,
                              bool octagonal) {
  if (firstCol >= kHomeAnalogGaugeWidth) {
    return;
  }
  if (lastCol >= kHomeAnalogGaugeWidth) {
    lastCol = kHomeAnalogGaugeWidth - 1;
  }
  for (uint8_t page = 0; page < kHomeAnalogGaugePages; ++page) {
    display.setCursor(firstCol, kHomePadRowOffset + page);
    for (uint8_t col = firstCol; col <= lastCol; ++col) {
      display.ssd1306WriteRamBuf(
        homeAnalogGaugeColumnByte(col, page, dotX, dotY, octagonal));
    }
  }
}

void writeHomeAnalogGaugeDotSpan(int8_t centerX,
                                 int8_t dotX, int8_t dotY,
                                 bool octagonal) {
  int16_t first = centerX - kHomeAnalogGaugeDotRadius;
  int16_t last = centerX + kHomeAnalogGaugeDotRadius;
  if (first < 0) first = 0;
  if (last >= kHomeAnalogGaugeWidth) last = kHomeAnalogGaugeWidth - 1;
  writeHomeAnalogGaugeSpan((uint8_t)first, (uint8_t)last,
                           dotX, dotY, octagonal);
}

void resetHomeAnalogGauge() {
  homeAnalogGaugeState = {};
}

void renderHomeAnalogGateGauge(const controller_state_t& frame,
                               bool forceRedraw) {
  const bool octagonal = analogTraceUsesOctagonalGate(
    deviceMode, frame.controller_type_name);
  const int16_t fullScale = homeAnalogGaugeFullScale(frame);
  const int8_t dotX = homeAnalogGaugeCoordinate(
    kHomeAnalogGaugeCenterX, frame.LX, fullScale);
  const int8_t dotY = homeAnalogGaugeCoordinate(
    kHomeAnalogGaugeCenterY, frame.LY, fullScale);
  const bool contextChanged =
    !homeAnalogGaugeState.initialized ||
    homeAnalogGaugeState.mode != deviceMode ||
    homeAnalogGaugeState.kind != HomeAnalogGaugeKind::Gate ||
    homeAnalogGaugeState.octagonal != octagonal ||
    homeAnalogGaugeState.fullScale != fullScale;

  if (forceRedraw || contextChanged) {
    writeHomeAnalogGaugeSpan(0, kHomeAnalogGaugeWidth - 1,
                             dotX, dotY, octagonal);
  } else if (homeAnalogGaugeState.dotX != dotX ||
             homeAnalogGaugeState.dotY != dotY) {
    writeHomeAnalogGaugeDotSpan(homeAnalogGaugeState.dotX,
                                dotX, dotY, octagonal);
    if (homeAnalogGaugeState.dotX != dotX) {
  ...
  homeAnalogGaugeState.initialized = true;
  homeAnalogGaugeState.mode = deviceMode;
  homeAnalogGaugeState.kind = HomeAnalogGaugeKind::Gate;
  homeAnalogGaugeState.octagonal = octagonal;
  homeAnalogGaugeState.fullScale = fullScale;
  homeAnalogGaugeState.dotX = dotX;
  homeAnalogGaugeState.dotY = dotY;
}

int16_t scaleHomeAnalogAxis(int16_t value, int16_t fullScale) {
  if (fullScale <= 0) {
    return 0;
  }
  int32_t clamped = value;
  if (clamped < -fullScale) clamped = -fullScale;
  if (clamped > fullScale) clamped = fullScale;
  return (int16_t)((clamped * 128) / fullScale);
}

void drawHomeDualAxisGaugeRow(const char* label, uint8_t row,
                              int16_t value, bool forceRedraw) {
  constexpr uint8_t kLabelWidth = 12;
  if (forceRedraw) {
    display.setFont(System5x7);
    display.setCursor(0, row);
    display.print(label);
  }
  drawCenterOutSignedAxisGauge(value, kLabelWidth, row,
                               kHomeAnalogGaugeWidth - kLabelWidth);
}

void renderHomeDualAxisGauges(const controller_state_t& frame,
                              bool forceRedraw) {
  const int16_t fullScale = analogTraceFullScale(frame.sticks_precision_bits);
  const int16_t axes[4] = {
    scaleHomeAnalogAxis(frame.LX, fullScale),
    scaleHomeAnalogAxis(-frame.LY, fullScale),
    scaleHomeAnalogAxis(frame.RX, fullScale),
    scaleHomeAnalogAxis(-frame.RY, fullScale),
  };
  const bool contextChanged =
    !homeAnalogGaugeState.initialized ||
    homeAnalogGaugeState.mode != deviceMode ||
    homeAnalogGaugeState.kind != HomeAnalogGaugeKind::DualAxes ||
    homeAnalogGaugeState.fullScale != fullScale;
  const bool redrawAll = forceRedraw || contextChanged;
  static const char* const labels[4] = { "LX", "LY", "RX", "RY" };

  for (uint8_t axis = 0; axis < 4; ++axis) {
    if (redrawAll || homeAnalogGaugeState.axes[axis] != axes[axis]) {
      drawHomeDualAxisGaugeRow(labels[axis], kHomePadRowOffset + axis,
                               axes[axis], redrawAll);
    }
  }
  ...
  homeAnalogGaugeState.mode = deviceMode;
  homeAnalogGaugeState.kind = HomeAnalogGaugeKind::DualAxes;
  homeAnalogGaugeState.octagonal = false;
  homeAnalogGaugeState.fullScale = fullScale;
  for (uint8_t axis = 0; axis < 4; ++axis) {
    homeAnalogGaugeState.axes[axis] = axes[axis];
  }
}

void renderHomeAnalogGauge(const controller_state_t& frame,
                           bool forceRedraw) {
  switch (homeAnalogGaugeKindForFrame(deviceMode, frame)) {
    case HomeAnalogGaugeKind::Gate:
      renderHomeAnalogGateGauge(frame, forceRedraw);
      break;
    case HomeAnalogGaugeKind::DualAxes:
      renderHomeDualAxisGauges(frame, forceRedraw);
      break;
    default:
      resetHomeAnalogGauge();
      break;
  }
}
...
bool isWiiClassicAnalogTriggerFrame(const controller_state_t* frame) {
#ifdef ENABLE_INPUT_WII
  return deviceMode == RZORD_WII &&
         frame != nullptr &&
         frame->connected &&
...
uint32_t analogTriggerGaugeMask(const controller_state_t* frame, bool left) {
  if (isWiiClassicAnalogTriggerFrame(frame)) {
    return left ? GPAD_L1 : GPAD_R1;
  }
  return left ? GPAD_L2 : GPAD_R2;
}

bool isAnalogTriggerMaskForFrame(const controller_state_t* frame,
                                 uint32_t mask) {
  return mask == analogTriggerGaugeMask(frame, true) ||
         mask == analogTriggerGaugeMask(frame, false);
}

bool isNeGconFrame(const controller_state_t* frame) {
  return frame != nullptr &&
         frame->connected &&
         std::strcmp(frame->controller_type_name, "neGcon") == 0;
}
...
bool shouldDrawAnalogTriggerGauge(const controller_state_t* frame,
  ...
  return frame != nullptr &&
         ...
         isAnalogTriggerMaskForFrame(frame, mask);
}

bool isAnalogTriggerGaugeButton(const PadButton& button,
                                const controller_state_t* frame) {
  return isAnalogTriggerMaskForFrame(frame, button.mask) &&
...
bool hasSeparateDigitalAnalogTriggerButton(const PadButton& button,
                                           ...
                                           uint8_t layoutCount,
                                           const controller_state_t* frame) {
  if (!isAnalogTriggerGaugeButton(button, frame) || layout == nullptr) {
    return false;
  }
  for (uint8_t i = 0; i < layoutCount; ++i) {
    ...
    if (&candidate != &button &&
        ...
        !isAnalogTriggerGaugeButton(candidate, frame)) {
      return true;
    }
  }
  return false;
...
uint8_t analogTriggerValue(const controller_state_t* frame,
  ...
  if (isNeGconFrame(frame)) {
    const uint8_t rawPressure =
      (mask == GPAD_L2) ? frame->ANALOG_X : frame->ANALOG_A;
    return rawPressure;
  }
  return (mask == analogTriggerGaugeMask(frame, true))
    ? frame->ANALOG_L2
...
uint8_t analogTriggerGaugeWidthForButton(const PadButton* layout,
  ...
  if (button.mask == analogTriggerGaugeMask(frame, true)) {
  ...
  } else if (button.mask == analogTriggerGaugeMask(frame, false)) {
  ...
  if (isNeGconFrame(frame)) {
    return availableWidth > kAnalogTriggerGaugeMinWidth + kNeGconAnalogGaugeGap
      ? availableWidth - kNeGconAnalogGaugeGap
      : kAnalogTriggerGaugeMinWidth;
  }
...
uint8_t analogTriggerGaugeWidthForMask(const PadButton* layout,
  ...
  for (uint8_t i = 0; i < layoutCount; ++i) {
    if (layout[i].mask == mask && isAnalogTriggerGaugeButton(layout[i], frame)) {
      return analogTriggerGaugeWidthForButton(layout, layoutCount, layout[i], frame);
    }
  }
  return kAnalogTriggerGaugeMinWidth;
}
...
int16_t padButtonColumnForFrame(const PadButton& button,
                                const PadButton* layout,
                                const controller_state_t* frame) {
  const bool spreadDualShockShoulders =
    layout == menu_pad_layouts_internal::padLayoutPSXDualShock &&
    frame != nullptr &&
    frame->connected &&
    !frame->HAS_ANALOG_TRIGGERS;
  if (!spreadDualShockShoulders) {
    return button.col;
  }
  if (button.mask == GPAD_L1) {
    return (int16_t)button.col - kPadGlyphWidth;
  }
  if (button.mask == GPAD_R1) {
    return (int16_t)button.col + kPadGlyphWidth;
  }
  return button.col;
}
...
void drawAnalogTriggerGauge(const PadButton& button,
  ...
  if (button.mask == analogTriggerGaugeMask(frame, false)) {
  ...
  for (uint8_t i = 0; i < gaugeWidth; ++i) {
    ...
    if (i == 0 || i == gaugeWidth - 1) {
      ...
      const bool fillFromRight = button.mask == analogTriggerGaugeMask(frame, true);
      const bool filled =
        fillFromRight
...
void drawCenterOutSignedAxisGauge(int16_t delta,
  ...
  const uint16_t boundedDelta = absDelta > 128 ? 128 : absDelta;
  ...
  uint8_t magnitude = (uint8_t)((boundedDelta * (uint16_t)halfSpan + 63) / 128);
...
void drawCenterOutPaddleGauge(const controller_state_t* frame,
  ...
  drawCenterOutSignedAxisGauge((int16_t)raw - 128,
                               col,
                               row,
                               panelWidth);
}

void drawNeGconTwistGauge(const controller_state_t* frame,
                          uint8_t colOffset) {
  const uint8_t endCol = (uint8_t)(colOffset + kNeGconTwistGaugeWidth - 1);
  display.clear(colOffset, endCol, kNeGconTwistGaugeRow, kNeGconTwistGaugeRow);
  drawCenterOutSignedAxisGauge(frame != nullptr ? frame->LX : 0,
                               colOffset,
                               kNeGconTwistGaugeRow,
                               kNeGconTwistGaugeWidth);
}
...
void drawPadSlotGlyph(const PadButton& button,
  ...
  const bool analogTriggerGaugeActive =
    isAnalogTriggerGaugeButton(button, analogFrame) &&
    shouldDrawAnalogTriggerGauge(analogFrame, button.mask);
  if (analogTriggerGaugeActive) {
    const uint8_t gaugeWidth =
      analogTriggerGaugeWidthForButton(layout, layoutCount, button, analogFrame);
  ...
  display.setFont(ReflexPad5x7);
  int16_t glyphCol = padButtonColumnForFrame(button, layout, analogFrame) + colOffset;
  int16_t glyphRow = (int16_t)button.row + rowOffset;
  if (analogTriggerGaugeActive && trigger_mode == TRIGGER_MODE_BOTH) {
    glyphRow += 1;
    if (button.mask == analogTriggerGaugeMask(analogFrame, true)) {
      glyphCol += (int16_t)kPadGlyphWidth * kTriggerBothGlyphInsetChars;
    } else if (button.mask == analogTriggerGaugeMask(analogFrame, false)) {
      glyphCol -= (int16_t)kPadGlyphWidth * kTriggerBothGlyphInsetChars;
    }
  }
  if (glyphCol < 0) {
...
bool shouldUseCenteredOutputButtonLabels() {
  return menu_home_button_labels != 0;
}

bool shouldUseSideBySideOutputButtonLabels() {
  return menu_home_button_labels != 0;
}
...
void renderPadTestPadView(bool centerHorizontally) {
#ifndef USE_I2C_DISPLAY
  return;
#else
  ...
  const PadButton* layout = nullptr;
  uint8_t layoutCount = 0;
  getLayoutForPlayer(0, &layout, &layoutCount);
  if (layout == nullptr || layoutCount == 0) {
    return;
  }

  display.setFont(ReflexPad5x7);
  drawPadLayoutState(layout,
                     layoutCount,
                     buildButtonMaskFromReport(0),
                     centerHorizontally ? kCenteredPadColOffset : 66,
                     centerHorizontally ? 2 : 1,
                     false,
                     false,
                     get_effective_output_mode(),
                     &controllerFrameConst(0));
  display.setFont(System5x7);
#endif
}
...
bool didControllerNamesChange() {
  static char lastDisplayedControllerType[2][16] = {{0}, {0}};
  bool changed = false;

  for (uint8_t player = 0; player < 2; ++player) {
    const char* currentType = "";
    if (player < max_devices && controllerFrameConst(player).connected) {
      currentType = controllerFrameConst(player).controller_type_name;
    }
    if (std::strncmp(lastDisplayedControllerType[player], currentType,
                     sizeof(lastDisplayedControllerType[player])) != 0) {
      std::strncpy(lastDisplayedControllerType[player], currentType,
                   sizeof(lastDisplayedControllerType[player]) - 1);
      lastDisplayedControllerType[player]
                                 [sizeof(lastDisplayedControllerType[player]) - 1] = '\0';
      changed = true;
    }
  }
  return changed;
...
void updateRealtimeButtons() {
  ...
  static bool lastShowHomeAnalogGauge = false;
  ...
  static int16_t lastNeGconTwist[2] = { 0x7FFF, 0x7FFF };
  ...
  const bool showHomeAnalogGauge =
    shouldShowHomeAnalogGauge(centerOutputVirtualPad);
  const uint8_t primaryPadColOffset = showHomeAnalogGauge
  ...
  bool secondPadVisibilityChanged =
    ...
    (lastShowHomeAnalogGauge != showHomeAnalogGauge) ||
  ...
  if (homeLayoutNeedsRedraw) {
    ...
    if (currentLayout[0] && !primaryLayoutDreamcastWheel) {
      if (!primaryLayoutSaturnWheel) {
        const bool useCenteredLabels =
          centerOutputVirtualPad &&
          shouldUseCenteredOutputButtonLabels();
    ...
    if (showSecondPad && currentLayout[1]) {
      const bool useSecondaryLabels =
        showOutputVirtualPad &&
        shouldUseSideBySideOutputButtonLabels();
    ...
    lastNeGconTwist[0] = 0x7FFF;
    lastNeGconTwist[1] = 0x7FFF;
  ...
  if (showHomeAnalogGauge && primaryAnalogFrame != nullptr) {
    renderHomeAnalogGauge(*primaryAnalogFrame, homeLayoutNeedsRedraw);
  } else {
    resetHomeAnalogGauge();
  }
  const bool primaryNeGcon =
    deviceMode == RZORD_PSX && isNeGconFrame(primaryAnalogFrame);
  const bool secondaryNeGcon =
    deviceMode == RZORD_PSX &&
    !showOutputVirtualPad &&
    isNeGconFrame(secondaryAnalogFrame);
  const bool primaryNeGconTwistChanged =
    primaryNeGcon && primaryAnalogFrame->LX != lastNeGconTwist[0];
  const bool secondaryNeGconTwistChanged =
    secondaryNeGcon && secondaryAnalogFrame->LX != lastNeGconTwist[1];
  #ifdef ENABLE_INPUT_DREAMCAST
  ...
  const uint32_t analogLeftGaugeMask[2] = {
    analogTriggerGaugeMask(primaryAnalogFrame, true),
    analogTriggerGaugeMask(secondaryAnalogFrame, true),
  };
  const uint32_t analogRightGaugeMask[2] = {
    analogTriggerGaugeMask(primaryAnalogFrame, false),
    analogTriggerGaugeMask(secondaryAnalogFrame, false),
  };
  const uint8_t analogL2GaugeWidth[2] = {
    primaryDreamcastWheel
      ? kDreamcastWheelTriggerGaugeWidth
      : analogTriggerGaugeWidthForMask(currentLayout[0], currentLayoutCount[0], analogLeftGaugeMask[0],
                                       primaryAnalogFrame),
    secondaryDreamcastWheel
      ? kDreamcastWheelTriggerGaugeWidth
      : analogTriggerGaugeWidthForMask(currentLayout[1], currentLayoutCount[1], analogLeftGaugeMask[1],
                                       secondaryAnalogFrame),
  };
  const uint8_t analogR2GaugeWidth[2] = {
    primaryDreamcastWheel
      ? kDreamcastWheelTriggerGaugeWidth
      : analogTriggerGaugeWidthForMask(currentLayout[0], currentLayoutCount[0], analogRightGaugeMask[0],
                                       primaryAnalogFrame),
    secondaryDreamcastWheel
      ? kDreamcastWheelTriggerGaugeWidth
      : analogTriggerGaugeWidthForMask(currentLayout[1], currentLayoutCount[1], analogRightGaugeMask[1],
                                       secondaryAnalogFrame),
  };
  const uint8_t analogL2Columns[2] = {
    analogTriggerGaugeColumns(primaryAnalogFrame, analogLeftGaugeMask[0], analogL2GaugeWidth[0]),
    analogTriggerGaugeColumns(secondaryAnalogFrame, analogLeftGaugeMask[1], analogL2GaugeWidth[1]),
  };
  const uint8_t analogR2Columns[2] = {
    analogTriggerGaugeColumns(primaryAnalogFrame, analogRightGaugeMask[0], analogR2GaugeWidth[0]),
    analogTriggerGaugeColumns(secondaryAnalogFrame, analogRightGaugeMask[1], analogR2GaugeWidth[1]),
  };
  ...
  const bool neGconTwistChanged =
    primaryNeGconTwistChanged || secondaryNeGconTwistChanged;
  if (!buttonsChanged && !analogChanged && !triggerRawDebugChanged &&
      !wheelAxisChanged && !neGconTwistChanged) {
    return;
  }
  if (!buttonsChanged && !triggerRawDebugChanged &&
      (analogChanged || wheelAxisChanged || neGconTwistChanged)) {
  ...
  } else if (analogChanged || wheelAxisChanged || neGconTwistChanged) {
    lastAnalogGaugeRedrawMs = millis();
  }

  display.setFont(ReflexPad5x7);
  const uint8_t primaryColOffset = primaryPadColOffset;
  ...
  if (!primaryDreamcastWheel &&
    ...
    for (uint8_t i = 0; i < currentLayoutCount[0]; ++i) {
      ...
      const bool triggerGaugeChanged =
        (mask == analogLeftGaugeMask[0] &&
         analogL2Columns[0] != lastAnalogL2[0]) ||
        (mask == analogRightGaugeMask[0] &&
         analogR2Columns[0] != lastAnalogR2[0]);
  ...
  if (primaryNeGconTwistChanged) {
    drawNeGconTwistGauge(primaryAnalogFrame, primaryColOffset);
  }
  ...
  if (showSecondPad &&
    ...
    for (uint8_t i = 0; i < currentLayoutCount[1]; ++i) {
      ...
      const bool triggerGaugeChanged =
        (mask == analogLeftGaugeMask[1] &&
         analogL2Columns[1] != lastAnalogL2[1]) ||
        (mask == analogRightGaugeMask[1] &&
         analogR2Columns[1] != lastAnalogR2[1]);
  ...
  if (showSecondPad && secondaryNeGconTwistChanged) {
    drawNeGconTwistGauge(secondaryAnalogFrame, kSecondPadColOffset);
  }
  ...
  lastWheelPaddle[1] =
    ((secondaryDreamcastWheel || secondarySaturnWheel) && secondaryAnalogFrame != nullptr)
      ...
      : 0xFF;
  lastNeGconTwist[0] =
    primaryNeGcon && primaryAnalogFrame != nullptr ? primaryAnalogFrame->LX : 0x7FFF;
  lastNeGconTwist[1] =
    secondaryNeGcon && secondaryAnalogFrame != nullptr ? secondaryAnalogFrame->LX : 0x7FFF;
