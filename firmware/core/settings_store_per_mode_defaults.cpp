...
uint8_t defaultButtonMapModeForInputMode(DeviceEnum mode) {
  switch (mode) {
    ...
    case RZORD_NES:      return 0;
  #endif
  #ifdef ENABLE_INPUT_SNES
    case RZORD_SNES:     return 1;
  #endif
  #ifdef ENABLE_INPUT_N64
    case RZORD_N64:      return 0;
  #endif
  #ifdef ENABLE_INPUT_GAMECUBE
    case RZORD_GAMECUBE: return 0;
  #endif
  #ifdef ENABLE_INPUT_WII
    case RZORD_WII:      return 1;
  #endif
  #ifdef ENABLE_INPUT_VBOY
    case RZORD_VBOY:     return 1;
  #endif
    default:             return 0;
  }
}
...
uint8_t defaultZButtonModeForInputMode(DeviceEnum mode) {
  switch (mode) {
  #ifdef ENABLE_INPUT_N64
    case RZORD_N64:      return 1;  // N64 Z defaults to L2/separate.
  #endif
  #ifdef ENABLE_INPUT_GAMECUBE
    case RZORD_GAMECUBE: return 0;  // GameCube Z defaults to RB/R1.
  #endif
    default:             return 1;
  }
}
...
uint8_t defaultN64CStickModeForInputMode(DeviceEnum mode) {
  #ifdef ENABLE_INPUT_N64
  if (mode == RZORD_N64) {
    return 1;  // N64 C buttons default to buttons on every selectable output.
  }
  #endif
  (void)mode;
  return 0;
...
void sanitizePerModeSettings(DeviceEnum mode, PerModeSettingsRecord& settings) {
  ...
  if (settings.rumble_level > RUMBLE_MODE_MAX) settings.rumble_level = defaultRumbleLevelForInputMode(mode);
  ...
  if (settings.nso_special > 1)
    settings.nso_special = defaultNsoSpecialForInputMode(mode);
...
#ifdef ENABLE_INPUT_GAMECUBE
  if (mode != RZORD_GAMECUBE ||
      settings.gamecube_l_switch_mode > GAMECUBE_L_SWITCH_L) {
    settings.gamecube_l_switch_mode = GAMECUBE_L_SWITCH_ZL;
  }
#else
  settings.gamecube_l_switch_mode = GAMECUBE_L_SWITCH_ZL;
#endif
#ifdef ENABLE_INPUT_WII
  if (settings.analog_mouse_mode > ANALOG_MOUSE_RIGHT) {
    settings.analog_mouse_mode = ANALOG_MOUSE_OFF;
  }
#else
  settings.analog_mouse_mode = ANALOG_MOUSE_OFF;
#endif
  if (settings.analog_mouse_speed < 1 ||
      settings.analog_mouse_speed > ANALOG_MOUSE_SPEED_MAX) {
    settings.analog_mouse_speed = ANALOG_MOUSE_SPEED_DEFAULT;
  }
