...
constexpr uint8_t HOST_OUTPUT_PREFERENCE_DINPUT = 0;
constexpr uint8_t HOST_OUTPUT_PREFERENCE_XINPUT = 1;
constexpr uint8_t HOST_OUTPUT_PREFERENCE_KEYBOARD = 2;
constexpr uint8_t WINDOWS_OUTPUT_PREFERENCE_MASK = 0x03;
constexpr uint8_t MISTER_OUTPUT_PREFERENCE_SHIFT = 2;
constexpr uint8_t MISTER_OUTPUT_PREFERENCE_MASK = 0x0C;

inline constexpr uint8_t windowsOutputPreferenceFromRaw(uint8_t raw) { return raw & WINDOWS_OUTPUT_PREFERENCE_MASK; }
inline constexpr uint8_t misterOutputPreferenceFromRaw(uint8_t raw) { return (raw & MISTER_OUTPUT_PREFERENCE_MASK) >> MISTER_OUTPUT_PREFERENCE_SHIFT; }
inline constexpr uint8_t encodeWindowsOutputPreference(uint8_t raw, uint8_t value) { return (raw & MISTER_OUTPUT_PREFERENCE_MASK) | (value & WINDOWS_OUTPUT_PREFERENCE_MASK); }
inline constexpr uint8_t encodeMisterOutputPreference(uint8_t raw, uint8_t value) { return (raw & WINDOWS_OUTPUT_PREFERENCE_MASK) | ((value << MISTER_OUTPUT_PREFERENCE_SHIFT) & MISTER_OUTPUT_PREFERENCE_MASK); }

static_assert(misterOutputPreferenceFromRaw(0) == HOST_OUTPUT_PREFERENCE_DINPUT, "Legacy Windows DInput setting must default MiSTer/Linux to DInput");
static_assert(misterOutputPreferenceFromRaw(1) == HOST_OUTPUT_PREFERENCE_DINPUT, "Legacy Windows XInput setting must default MiSTer/Linux to DInput");
static_assert(misterOutputPreferenceFromRaw(2) == HOST_OUTPUT_PREFERENCE_DINPUT, "Legacy Windows keyboard setting must default MiSTer/Linux to DInput");
static_assert(encodeWindowsOutputPreference(0x08, 1) == 0x09, "Windows preference must preserve MiSTer/Linux preference");
static_assert(encodeMisterOutputPreference(0x02, 1) == 0x06, "MiSTer/Linux preference must preserve Windows preference");
...
struct PerModeSettingsRecord {
  ...
  // Reuse the retired mouse-analog byte without changing the EEPROM layout.
  uint8_t gamecube_l_switch_mode;
  uint8_t analog_mouse_mode;
  uint8_t analog_mouse_speed;
  uint8_t jogcon_mode;
...
uint8_t defaultN64CStickModeForInputMode(DeviceEnum mode);
...
uint8_t defaultNsoSpecialForInputMode(DeviceEnum mode);
