...
class MPG
{
	public:
		...
		/**
		 * @brief Check for a button press. Using the aux bits
		 */
		inline bool __attribute__((always_inline)) pressedButtonAux(const uint8_t index, const uint16_t mask) { return (state[index].aux & mask) == mask; }
		...
		bool __attribute__((always_inline)) pressedUp(const uint8_t index)    { return pressedDpad(index, GAMEPAD_MASK_UP); }
		bool __attribute__((always_inline)) pressedDown(const uint8_t index)  { return pressedDpad(index, GAMEPAD_MASK_DOWN); }
		bool __attribute__((always_inline)) pressedLeft(const uint8_t index)  { return pressedDpad(index, GAMEPAD_MASK_LEFT); }
		bool __attribute__((always_inline)) pressedRight(const uint8_t index) { return pressedDpad(index, GAMEPAD_MASK_RIGHT); }
		bool __attribute__((always_inline)) pressedB1(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_B1); }
		bool __attribute__((always_inline)) pressedB2(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_B2); }
		bool __attribute__((always_inline)) pressedB3(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_B3); }
		bool __attribute__((always_inline)) pressedB4(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_B4); }
		bool __attribute__((always_inline)) pressedL1(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_L1); }
		bool __attribute__((always_inline)) pressedR1(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_R1); }
		bool __attribute__((always_inline)) pressedL2(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_L2); }
		bool __attribute__((always_inline)) pressedR2(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_R2); }
		bool __attribute__((always_inline)) pressedS1(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_S1); }
		bool __attribute__((always_inline)) pressedS2(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_S2); }
		bool __attribute__((always_inline)) pressedL3(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_L3); }
		bool __attribute__((always_inline)) pressedR3(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_R3); }
		bool __attribute__((always_inline)) pressedA1(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_A1); }
		bool __attribute__((always_inline)) pressedA2(const uint8_t index)    { return pressedButton(index, GAMEPAD_MASK_A2); }
		bool __attribute__((always_inline)) pressedF1(const uint8_t index)    { return pressedButton(index, f1Mask); }
		bool __attribute__((always_inline)) pressedF2(const uint8_t index)    { return pressedButton(index, f2Mask); }
