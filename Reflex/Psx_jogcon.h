...
void handleJogconData()
{
  ...
  if(!gotJogconData) {//(data[0] == 0xF3)
...
#if defined REFLEX_USE_OLED_DISPLAY && defined ENABLE_PSX_JOGCON_OLED
    /*display.setCursor(0, oledDisplayFirstRow + 1);
    display.clearToEOL();
    switch(mode){
      case 0: display.print(F("SPINNER")); break;
      case 1: display.print(F("PADDLE")); break;
      case 2: display.print(F("WHEEL")); break;
      default: display.print(F("FAKE SPINNER")); break;
    }*/
    switch(mode){
      case 0: clearOledLineAndPrint(0, 1, F("SPINNER")); break;
      case 1: clearOledLineAndPrint(0, 1, F("PADDLE")); break;
      case 2: clearOledLineAndPrint(0, 1, F("WHEEL")); break;
      default: clearOledLineAndPrint(0, 1, F("FAKE SPINNER")); break;
    }
    ...
    clearOledLineAndPrint(0, 2, F("FORCE "));
    display.print(force);

    //display.setCursor(0, oledDisplayFirstRow + 3);
    //display.clearToEOL();
    //display.print(F("SP STEP "));
    clearOledLineAndPrint(0, 3, F("SP STEP "));
    display.print(sp_step);

    //display.setCursor(0, oledDisplayFirstRow + 4);
    //display.clearToEOL();
    //display.print(F("RANGE "));
    clearOledLineAndPrint(0, 4, F("RANGE "));
    display.print(sp_div == 1 ? 270 : 135 );
