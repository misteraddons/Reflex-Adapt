...
def main() -> int:
    ...
    assert manager.setting_name(42) == "JogCon Force"
    assert manager.setting_name(35) == "GunCon X Alignment"
    assert manager.setting_name(36) == "GunCon Y Alignment"
    assert manager.setting_name(43) == "JogCon Range"
    assert manager.setting_name(44) == "JogCon Digital Output"
    assert manager.setting_name(29, 9) == "N64 Z Output"
    assert manager.setting_name(29, 10) == "GameCube Z Output"
    assert manager.setting_name(38, 10) == "L Shoulder"
    assert manager.setting_choices({"id": 29, "mode": 9}) == ((0, "L1"), (1, "L2"))
    assert manager.setting_choices({"id": 29, "mode": 10}) == ((0, "R1"), (1, "Back"))
    assert manager.setting_choices({"id": 38, "mode": 10}) == ((0, "ZL"), (1, "L"))
    ...
    assert manager.setting_value_label({"id": 42, "value": 15}) == "High"
    assert manager.setting_value_label({"id": 44, "value": 2}) == "L1 / R1"
    assert manager.setting_value_label({"id": 54, "value": 1}) == "On"
    assert len(manager.SETTING_NAMES) == 59
    assert "Reserved PSX Slot" not in manager.SETTING_NAMES
    assert {0, 1, 3, 4, 5, 8, 9, 15, 16, 17, 18, 19, 22, 37, 39, 40, 41}.isdisjoint(
    ...
    assert 56 in manager.GLOBAL_HOTKEY_SETTING_IDS
    ...
    assert 31 not in manager.manager_input_setting_ids(10, {29, 31, 54})
    assert 31 not in manager.manager_input_setting_ids(11, {31, 54})
    assert manager.manager_input_setting_ids(9, {29, 32}) == {29, 32}
    assert manager.manager_input_setting_ids(10, {29, 38}) == {29, 38}
    assert manager.manager_input_setting_ids(9, {29, 38}) == {29}
    ...
    class HostOutputPicker:
        def __init__(self):
            self.results = iter((0, 1, None))

        def choose(self, title, options):
            assert title == "Host Output Modes"
            assert "Windows Output" in options[0]
            assert "MiSTer/Linux Output" in options[1]
            return next(self.results)

    host_output_calls = []
    original_read_classic_settings = manager.read_classic_settings
    original_edit_setting = manager.edit_setting
    manager.read_classic_settings = lambda _port_name, _mode: [
        {"id": 15, "value": 0},
        {"id": 58, "value": 0},
    ]
    manager.edit_setting = (
        lambda _ui, _port_name, setting: host_output_calls.append(setting["id"])
    )
    try:
        manager.host_output_modes_flow(HostOutputPicker(), "/dev/ttyACM0", 6)
    finally:
        manager.read_classic_settings = original_read_classic_settings
        manager.edit_setting = original_edit_setting
    assert host_output_calls == [15, 58]
