...
def audit() -> list[str]:
    ...
    for token, label in (
        ("rumblePayload != nullptr && rumblePayloadSize >= 4", "Stadia DInput report length guard"),
        ("const uint16_t weak = (uint16_t)rumblePayload[2] |", "Stadia weak-motor decode"),
    ...
    rumble_caps = read("firmware/menu/rumble_capabilities.h")
    require(
    ...
    post_build_validations = targets["targets"]["classic2usb"].get(
        "post_build_validation_commands", []
    )
    require(any(
        "test_classic2usb_link_isolation.py" in " ".join(command)
        for command in post_build_validations
    ), "Classic2USB package must reject ESP32-only linked libraries", failures)
    require("post_build_validation_commands" in package_release,
            "Package builder must run post-build artifact validation", failures)
