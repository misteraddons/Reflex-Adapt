...
def source_guards() -> None:
    ...
    rumble_runtime = (
        ROOT / "firmware" / "core" / "rumble_test_runtime.cpp"
    ).read_text(encoding="utf-8")
    descriptors = (
        ROOT / "firmware" / "menu" / "menu_descriptors_data.cpp"
    ).read_text(encoding="utf-8")
    required = (
        ...
        (rumble_runtime, "if (rumble_level == RUMBLE_MODE_OFF || value == 0) return 0;"),
        (rumble_runtime, "if (rumble_level == RUMBLE_MODE_MAX) return 255;"),
        (rumble_runtime, "return value;"),
        (descriptors, 'rumble_names[] = { "Off", "Variable", "Max" };'),
        (descriptors, ".enum_cfg = { rumble_names, 3, nullptr }"),
    )
