...
def audit() -> tuple[list[str], list[str]]:
    ...
    require(len(adapt_modes) == 23, f"Adapt.html fallback output catalog should expose 23 Classic2USB entries, found {len(adapt_modes)}", failures)
    ...
    special_offsets = {
        "WinOutput": "win_output",
        "MisterOutput": "win_output",
    ...
    require(defaults.get("WinOutput") == "0", "Windows output preference must default to DInput", failures)
    require(defaults.get("MisterOutput") == "0", "MiSTer/Linux output preference must default to DInput", failures)
    ...
    require("test_rumble_capabilities.py" in check_runner, "Unified checks must run rumble capability tests", failures)
    require("test_auto_input_usb_identity.py" in check_runner, "Unified checks must run Auto input identity tests", failures)
    require("test_host_output_preferences.py" in check_runner, "Unified checks must run host output preference tests", failures)
