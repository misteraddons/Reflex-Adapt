...
void testGatePolicyByControllerType() {
  ...
  check(analogTraceUsesOctagonalGate(RZORD_WII, "Nunchuk"),
        "Wii Nunchuk should use an octagonal trace");
...
void testOctagonalDirectionalCapture() {
  ...
  trace.sample(2, -3, true, 7, 127);
  trace.sample(0, -85, true, 7, 127);
  ...
  trace.sample(70, -70, true, 7, 127);
  trace.sample(85, 0, true, 7, 127);
  trace.sample(70, 70, true, 7, 127);
  trace.sample(0, 85, true, 7, 127);
  trace.sample(-70, 70, true, 7, 127);
  trace.sample(-85, 0, true, 7, 127);
  trace.sample(-70, -70, true, 7, 127);
  trace.sample(30, -30, true, 7, 127);
...
void testRoundCaptureUsesCardinalExtrema() {
  AnalogStickTrace trace;
  trace.sample(90, -90, false, 7, 127);
  trace.sample(0, -127, false, 7, 127);
  trace.sample(127, 0, false, 7, 127);
  trace.sample(0, 127, false, 7, 127);
  trace.sample(-127, 0, false, 7, 127);
...
void testDirectionalCaptureKeepsFarthestSample() {
  AnalogStickTrace trace;
  trace.sample(0, -80, true, 7, 127);
  trace.sample(0, -85, true, 7, 127);
  trace.sample(0, -82, true, 7, 127);
  checkPoint(trace, AnalogTraceDirection::Up, 0, -85,
             "directional capture should keep the farthest sample");
}

void testDotTracePreservesVisitedPath() {
  AnalogStickTrace trace;
  check(trace.sample(127, -127, true, 7, 127),
        "first corner sample should change the dot trace");
  check(trace.dotAt(50, 0),
        "full-scale corner must remain as its own dot");
  check(trace.sample(64, -64, true, 7, 127),
        "a distinct inner path sample should add another dot");
  check(trace.dotAt(37, 13),
        "inner path sample must not be averaged into the corner");
  check(!trace.sample(64, -64, true, 7, 127),
        "an identical dot and weaker maximum should not dirty the trace");
  checkPoint(trace, AnalogTraceDirection::UpRight, 127, -127,
             "dot tracing must retain numeric directional extrema");
  check(!trace.dotAt(AnalogStickTrace::kPlotDiameter, 0),
        "dot lookup must reject coordinates outside the plot");
  check(trace.dotCount() == 2,
        "trace should count only distinct retained dots");
  trace.reset();
  check(!trace.dotAt(50, 0) && !trace.dotAt(37, 13),
        "reset must clear the sampled dot path");
  check(trace.dotCount() == 0,
        "reset must clear the retained dot count");

  AnalogStickTrace sparse;
  sparse.sample(10, 10, true, 0, 25);
  sparse.sample(11, 10, true, 0, 25);
  check(sparse.dotCount() == 1 && !sparse.dotAt(35, 35) &&
        sparse.dotAt(36, 35),
        "an adjacent outward sample must replace the inner dot");
  sparse.sample(10, 12, true, 0, 25);
  check(sparse.dotCount() == 2 && sparse.dotAt(35, 37),
        "separated samples must remain as true dots");

  AnalogStickTrace capacity;
  for (int16_t y = -25;
       y <= 25 && capacity.dotCount() < AnalogStickTrace::kMaxPlotDots;
       y += 2) {
    for (int16_t x = -25;
         x <= 25 && capacity.dotCount() < AnalogStickTrace::kMaxPlotDots;
         x += 2) {
      capacity.sample(x, y, true, 0, 25);
    }
  }
  check(capacity.dotCount() == AnalogStickTrace::kMaxPlotDots,
        "dot trace must retain its full 500-dot capacity");
  capacity.sample(25, 25, true, 0, 25);
  check(capacity.dotCount() == AnalogStickTrace::kMaxPlotDots &&
        !capacity.dotAt(50, 50),
        "dot trace must reject new separated dots after reaching capacity");
}

void testInt16ProjectionAndReset() {
  AnalogStickTrace trace;
  trace.sample(INT16_MIN, INT16_MIN, true, 100, INT16_MAX);
...
int main() {
  ...
  testDotTracePreservesVisitedPath();
