...
Two System Settings control the final transport selected by **Auto** output:

| Setting | Options | Default | Used When |
|---------|---------|---------|-----------|
| Windows Mode | DInput, XInput, Keyboard | DInput | Windows is positively identified |
| MiSTer Mode | DInput, XInput, Keyboard | DInput | No Windows or console signature is found, including MiSTer and typical Linux hosts |

MiSTer Mode is intentionally a generic-host fallback rather than proof that the
host is MiSTer. Selecting XInput or Keyboard for either preference can remove
the normal WebHID, serial-management, and utility-drive interfaces until the
preference is returned to DInput from the OLED menu.

The XInput choice is host-specific. A positively identified Windows host uses
the four-child Xbox 360 wireless-receiver personality, with up to two active
Classic2USB physical inputs. MiSTer/Linux uses two independent wired XInput
interfaces and does not require a custom MiSTer kernel or zImage_dtb.
...
| Genesis / Mega Drive | 3-button and 6-button controllers; 8BitDo M30 2.4G in 6-button mode (identified as M30 after Home/Heart or Star is observed) |
...
| XInput (MiSTer/Linux or manual) | MiSTer, Linux | 2 | Two wired interfaces; either physical port may become P1 |
| XInput (Windows Auto) | Windows | 2 active / 4 slots | Four-child receiver personality; Classic2USB has two physical inputs |
...
| XInput PC | Two-player receiver path; live ports compact into P1/P2 without USB re-enumeration, and rumble follows the assigned source; separate from Xbox 360 console authentication |
...
Holding the physical left/reset button for 1.5 seconds while any menu is open returns to the Home screen and discards unsaved changes.
...
| Analog mouse | In DInput/MiSTer output, the Analog quick menu offers Mouse `Off`/`Joystick` for one-stick controllers and `Off`/`Left Joy`/`Right Joy` for dual-stick controllers, plus shared `Pointer Speed` 1-16. Primary face buttons act as left/right click. |
...
Analog Range offers `Raw`, `Norm`, `Cal`, and `Learn`. `Learn` is the factory
default: each stick axis and analog trigger begins at a conservative
controller-family baseline and expands when a larger value is observed. Ordinary
movement never shrinks a learned range. On GameCube controllers, the digital
trigger click is also treated as proof of full travel, so a controller whose
analog potentiometer tops out early can still fill the OLED gauge and output
range correctly. Use `Cal` for a deliberate guided calibration, `Norm` for the
fixed controller-family range, or `Raw` for unscaled values.

Variable-strength controllers expose **Rumble: Off / Variable / Max**.
`Variable` is the factory default and preserves the strength requested by the
host independently for each motor. `Max` promotes every nonzero motor request
to full strength while preserving which motor was requested; it is useful when
a binary-era game or core produces effects too weak for a modern variable motor.
Binary-rumble controllers continue to expose only `Off / On`.
...
Engine, Saturn, Genesis / Mega Drive, 3DO, PlayStation, Wii, and Dreamcast.
...
Output Auto presents a probe device, identifies hosts with a distinctive
Windows or console signature, and reboots into the selected runtime output
mode. Windows uses the saved **Windows Mode** preference. A generic host with no
positive Windows or console signature uses the saved **MiSTer Mode** preference;
this fallback includes MiSTer and typical Linux systems but may also cover other
generic USB hosts. Both preferences default to DInput, and the saved output mode
remains Auto unless changed in the menu.
...
The Windows and MiSTer/Linux measurements above use the default DInput
preferences. Changing a preference changes the final USB personality, not which
host-detection branch selected it.
...
prints debug commands, and `CAPS` or `MGMT HELP` prints the machine-oriented
management surface. Commands are case-insensitive. Destructive commands require
the literal final word `CONFIRM`; trailing text is rejected.
...
| `CAPS`, `MGMT HELP` | List supported management command families |
| `STATUS` | Short firmware/runtime status |
| `STATE` | Full runtime state snapshot |
| `PLAYER LIST` | List the six possible Classic2USB output slots, connection state, and controller type |
| `GPIO` | GPIO diagnostics |
| `RESET`, `REBOOT` | Reboot the adapter |
| `BOOT`, `BOOTLOADER`, `B` | Reboot into RP2040 bootloader |
| `INPUT STATUS/LIST/SET <MODE>` | Inspect or persist a supported input mode; changing mode reboots |
| `OUTPUT STATUS/LIST/SET <MODE>` | Inspect or persist a supported output mode; changing mode reboots |
| `OUT <MODE>` | Legacy output-mode setter |
| `WIN <MODE>` | Set Windows auto-output preference |
| `MISTER <MODE>` | Set the MiSTer/Linux generic-host auto-output preference |

### Settings, Hotkeys, and Remaps

| Command | Purpose |
|---------|---------|
| `SET LIST [MODE]` | List all 60 Classic2USB settings, names, limits, and values |
| `SET VISIBLE [MODE] [OUT]` | List settings visible for a mode/output pair |
| `SET GET <ID\|NAME> [MODE]` | Read one setting by numeric ID or stable name |
| `SET SET <ID\|NAME> <VALUE> [MODE]` | Save and read back one setting; reports whether it applied live or requires reboot |
| `SET DEFAULT <ID\|NAME> [MODE]` | Restore, read back, and apply one setting where possible |
| `SET FACTORY_RESET CONFIRM` | Clear settings, hotkeys, remaps, and calibration data; preserve PS4 auth keys |
| `CONFIG`, `SETTING`, `SETTINGS` | Aliases for the `SET` command family |
| `TURBO LIST/SET`, `TURBO CLEAR CONFIRM` | Inspect or change turbo for the active input mode |
| `REMAP LIST/SET`, `REMAP CLEAR CONFIRM` | Inspect or change button remaps for the active input mode |
| `HOTKEY LIST/GET/SET`, `HOTKEY DEFAULTS CONFIRM` | Manage menu/system/home/capture hotkeys |
| `CHORD LIST/SET`, `CHORD CLEAR <SLOT\|ALL> CONFIRM` | Manage chord remap slots |
...
| `RUMBLE TEST PLAYER <1-6> <LEFT> <RIGHT> [MS]` | Test one Classic2USB output slot |
| `RUMBLE STOP` | Stop rumble test |
| `LED STATUS` | Read device RGB LED mode and brightness |
| `LED MODE <OFF\|STATIC\|BREATHING\|RAINBOW\|REACTIVE\|RUMBLE\|ANALOG>` | Save and apply the device LED mode |
| `LED BRIGHTNESS <0-254>` | Save and apply device LED brightness |
...
| `MENU CONTROLLER`, `MENU SYSTEM`, `MENU CLOSE` | Open or close a menu tier directly; unsaved quick-menu edits are discarded when switching tiers |
...
| `AUTH STATUS`, `AUTH CLEAR PS4 CONFIRM` | Inspect or clear stored PS4 authentication state without exposing key material |
| `STATS LIST`, `STATS CLEAR CONFIRM` | Inspect or clear button/poll statistics |
| `HISTORY LIST`, `HISTORY CLEAR CONFIRM` | Inspect or clear the input-history ring |

The commands listed here are the complete Classic2USB management surface.
Radio, network, USB-controller-host, learned-profile, transport-persona, and
controller-LED command families are intentionally not advertised.

## MiSTer Maps

For **Auto** output, the **MiSTer Mode** preference defaults to DInput. DInput is
recommended for MiSTer's normal input menu and GameControllerDB mapping.
The virtual utility drive is present on every Windows/MiSTer DInput identity,
including the DInput stage of Auto probing. WebHID and serial management become
available after Auto resolves, or when DInput is selected manually. XInput and
Keyboard remain explicit alternatives.
MiSTer GameControllerDB entries are maintained outside this firmware
...
Classic2USB's utility drive is a small virtual FAT volume generated by the
firmware, not a physical flash partition. It appears on every Windows/MiSTer
DInput identity and includes `ADAPTDL.INI`. Copy that file to
...
- The virtual utility drive is available on every Windows/MiSTer DInput
  identity, including Auto's DInput probe stage, plus specialized MiSTer PSX
  modes. WebHID and USB serial management remain hidden only during active Auto
  probing; they become available after Auto resolves or when DInput is selected
  manually.
