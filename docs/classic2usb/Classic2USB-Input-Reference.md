...
| 8BitDo M30 2.4G | Supported as 6-button-style input | Appears as Mega6 until Home/Heart or Star is pressed, then identifies as M30 until disconnected |
...
| JogCon | Supported | Quick menu selects Spinner, Paddle, Wheel, or Fake Spinner |
...
Known limitation: PlayStation physical port 2 may not initialize when it is the
only controller connected from an empty state. Connect port 1 first when using
port 2; after both ports initialize, either controller can operate independently.

Use **JogCon Mode** in the quick menu. The original Adapt/RetroZord shortcuts
remain available: hold a button combination while pressing JogCon MODE. These
shortcuts update the same live mode selection but are not native JogCon modes.
...
Spinner and Fake Spinner expose **Spin Sens** (0.25x through 4x) in the quick
menu. The same sensitivity also scales Spinner's analog-stick fallback so it
does not reach full deflection after only a small turn. Paddle and Wheel expose
**Force** and **Wheel Sens**; Force defaults to High. Paddle applies the
JogCon motor brake at its endpoints, while Wheel actively returns toward the zero
captured when the mode is selected. Fake Spinner defaults to D-pad Left/Right;
**Dig Output** can select an alternate button pair. On MiSTer DInput, JogCon
automatically uses its specialized relative-spinner/absolute-paddle USB report.
...
N64 C buttons default to `Buttons` in every output mode, and `Auto` follows that
same independent-button policy. DInput keeps the four neutral backing buttons.
XInput uses the Adapt V1 layout: C-Down/C-Right/C-Left/C-Up map to Xbox
`A/B/X/Y`, physical N64 `B/A` map to Xbox `LT/RT`, `Z` maps to `Back`, and
`L/R` map to `LB/RB`. Select `Stick` to move the C buttons to the four
full-scale directions of the right stick while retaining the other Adapt V1
assignments. Native N64 NSO mode keeps the protocol-required independent buttons.
...
|------------|-----------------|-------|
| Virtual Boy | Supported | DInput keeps both D-pads independent; XInput maps physical A/B to Xbox A/B and the right D-pad to the right stick |
...
With Output Auto selected, Windows Mode and MiSTer Mode independently choose
DInput, XInput, or Keyboard; both default to DInput. MiSTer Mode names the
generic-host fallback, not a positive MiSTer fingerprint. It is used whenever
no Windows or console signature is found, including MiSTer, typical Linux
systems, and potentially other generic USB hosts. Non-DInput preferences do not
provide the normal WebHID, serial-management, or utility-drive interfaces.
...
Output Auto presents a probe device, identifies hosts with a positive Windows
or console signature, and reboots into the selected runtime output mode.
Windows uses Windows Mode; an unidentified generic host uses MiSTer Mode. Both
preferences default to DInput. The saved output mode remains Auto unless changed
in the menu.

## MiSTer HID Mapping

DInput is the recommended MiSTer Mode preference. It uses a HID report with
stable button and axis indices and retains the management interfaces and
MiSTer-specific peripheral paths.
