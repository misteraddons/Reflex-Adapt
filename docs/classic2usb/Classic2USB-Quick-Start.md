...
4. Use **Auto** output mode for host detection. Windows Mode and MiSTer Mode both
   default to DInput, the broadest PC/MiSTer setup and USB serial-management
   choice.
...
System Settings exposes separate **Windows Mode** and **MiSTer Mode** Auto-output
preferences, each with DInput, XInput, and Keyboard choices. MiSTer Mode is used
for the generic-host fallback when Auto finds no Windows or console signature;
this includes MiSTer and typical Linux hosts. Every Windows/MiSTer DInput
identity exposes the virtual utility drive, including DInput while Auto is
probing. WebHID and serial management remain hidden during the probe stage and
become available after Auto resolves, or when DInput is selected manually.
...
For MiSTer, use **Auto** output with **MiSTer Mode = DInput** (the default), or
select DInput manually. Auto treats a host without a positive Windows or console
signature as the MiSTer/Linux generic-host fallback. Map controls through
MiSTer's normal input menu.

For supported classic analog inputs, **Analog Range = Learn** is the default. It learns each
stick axis and trigger independently, expands as larger values are observed,
and does not shrink during ordinary movement. A GameCube trigger's digital
click confirms full travel even when its analog potentiometer has a short
range. `Cal` starts guided calibration; `Norm` uses a fixed family range; `Raw`
leaves values unscaled.

Variable-motor controllers offer **Rumble = Off / Variable / Max**. Keep the
default `Variable` to preserve game-requested strength. Use `Max` when a core's
nonzero rumble is too weak; it does not merge the independent motor channels.

For managed installation, add the packaged `reflex-adapt-manager.ini` entry to
any `/media/fat/*downloader.ini` file and run Downloader. The Classic2USB
utility drive appears on every Windows/MiSTer DInput identity. It is a
firmware-generated virtual FAT volume, not a physical partition, and contains
the same entry as `ADAPTDL.INI`; copy it to
