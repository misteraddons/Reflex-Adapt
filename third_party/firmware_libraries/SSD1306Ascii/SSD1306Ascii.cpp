...
//------------------------------------------------------------------------------
void SSD1306Ascii::init(const DevType* dev, bool clearDisplay) {
  m_col = 0;
  m_row = 0;
#ifdef __AVR__
...
void SSD1306Ascii::ssd1306WriteRam(uint8_t c) {
  if (m_col < m_displayWidth) {
    const uint8_t value = c ^ m_invertMask;
    if (reflex_oled_mirror_ram_write) {
      reflex_oled_mirror_ram_write(m_col, m_row, value);
    }
    writeDisplay(value, SSD1306_MODE_RAM);
    m_col++;
  }
}
//------------------------------------------------------------------------------
void SSD1306Ascii::ssd1306WriteRamBuf(uint8_t c) {
  if (m_skip) {
    ...
    writeDisplay(value, SSD1306_MODE_RAM_BUF);
    m_col++;
  }
}
//------------------------------------------------------------------------------
