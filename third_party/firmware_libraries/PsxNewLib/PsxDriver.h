...
protected:
	 ...
	 */
	uint16_t attnInterval = MIN_ATTN_INTERVAL;
...
public:
	 ...
	 * \sa MIN_ATTN_INTERVAL
	 */
	void setAttentionInterval (const uint16_t us) {
		attnInterval = us;
	}
	 ...
	 */
	uint16_t getAttentionInterval () const {
		return attnInterval;
	}

	virtual void selectController () {
		while (micros () - lastCmdTime <= attnInterval) // millis ()
			;
	...
	boolean shiftInOut (const byte *out, const byte outLen,
		...
		return shiftInOutTimed (out, outLen, in, len, needLastAck, INTER_CMD_BYTE_TIMEOUT);
	}

	/** \brief Transfer several bytes with a caller-selected ACK timeout.
	 *
	 * Memory card reads can hold /ACK longer than controller polling after the
	 * address phase, so Adapt's card bridge uses this path without changing the
	 * normal controller timing.
	 */
	boolean shiftInOutTimed (const byte *out, const byte outLen,
	                         byte *in,  const byte len,
	                         const boolean needLastAck,
	                         const uint16_t ackTimeoutUs) {
		...
		for (byte i = 0; i < len; ++i) {
			...
			if (i < len - 1 || needLastAck) {
				...
				while (!acknowledged () && micros () - start < ackTimeoutUs)
					;
				if (!acknowledged ()) {
					ret = false;
				}
	...
	/** \brief Transfer bytes with one command-specific longer ACK window.
	 *
	 * PS1 memory-card reads have a documented late /ACK after the read command
	 * acknowledge byte. Keeping every byte on that long timeout can stretch
	 * early missed ACKs enough for some cards to abort, so callers can keep the
	 * normal controller timeout for most bytes and opt one byte into the longer
	 * window.
	 */
	boolean shiftInOutTimedWithLateAck (const byte *out, const byte outLen,
	                                    byte *in,  const byte len,
	                                    const boolean needLastAck,
	                                    const uint16_t ackTimeoutUs,
	                                    const byte lateAckAfterByte,
	                                    const uint16_t lateAckTimeoutUs) {
		boolean ret = true;
		...
		for (byte i = 0; i < len; ++i) {
			...
			if (i < len - 1 || needLastAck) {
				const uint16_t timeout = (i == lateAckAfterByte) ? lateAckTimeoutUs : ackTimeoutUs;
				unsigned long start = micros ();
				while (!acknowledged () && micros () - start < timeout)
					;
				if (!acknowledged ()) {
					ret = false;
				}
