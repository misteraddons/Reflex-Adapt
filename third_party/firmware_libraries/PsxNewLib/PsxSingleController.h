...
public:
	...
	boolean read () {
		...
		// Always clock the motor slots with explicit zeroes. Some controllers keep
		// their rumble-byte mapping across resets or a failed disable command, so
		// allowing autoShift() to pad a short poll with 0x5A can twitch the motors.
		out[3] = 0x00;
		out[4] = 0x00;
