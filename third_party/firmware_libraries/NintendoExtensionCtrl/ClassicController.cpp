...
boolean ClassicControllerBase::specificInit() {
	...
	uint8_t idData[ID_Size];//ID_Size
	if (!requestIdentity(idData)) {
	return false;
	}

	if (idData[0] == 0x00 && idData[1] == 0x00 && idData[4] <= 0x03 && idData[5] == 0x01) {
		classicPro = false;
	} else if (idData[0] == 0x01 && idData[1] == 0x00 && idData[4] <= 0x03 && idData[5] == 0x01) {
		classicPro = true;
	}
...
boolean ClassicControllerBase::checkDataMode(boolean *hr) const {
	...
	static const uint8_t MaxRetries = 10;    // max retries to prevent infinite loop on noisy I2C
...
boolean ClassicControllerBase::isClassicPro() const {
	return classicPro;
}
