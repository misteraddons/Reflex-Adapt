...
boolean ExtensionController::adoptIdentity(const uint8_t* idData) {
	if (idData == nullptr) return false;
	data.connectedType = decodeIdentity(idData);
	memset(&data.controlData, 0x00, ExtensionData::ControlDataSize);
	data.requestSize = MinRequestSize;
	return controllerTypeMatches();
}
...
boolean ExtensionPort::connectFromIdentity(const uint8_t* idData) {
	boolean success = adoptIdentity(idData);
