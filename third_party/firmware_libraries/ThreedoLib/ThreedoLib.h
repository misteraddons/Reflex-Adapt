...
class ThreedoPort {
  private:
    const uint8_t tdo_CLOCK; //DigitalPin<PIN_CLOCK> TDO_CLOCK;
    const uint8_t tdo_DOUT; //DigitalPin<PIN_DOUT> TDO_DOUT;
    const uint8_t tdo_DIN; //DigitalPin<PIN_DIN> TDO_DIN;
    uint8_t joyCount = 0;
    ThreedoController controllers[THREEDO_MAX_CTRL];

    //inline void __attribute__((always_inline))
    void __not_in_flash_func(doClockCycle)() {
      //Hold HI/LOW for 4us each
      //Data can be read at falling edge
      gpio_put(tdo_CLOCK, HIGH);//TDO_CLOCK.write(HIGH);
      delayMicroseconds(4);
      gpio_put(tdo_CLOCK, LOW);//TDO_CLOCK.write(LOW);
      delayMicroseconds(4);
    }
    ...
    void __not_in_flash_func(readControllers)(const uint8_t maxControllers) {
      ...
      gpio_put(tdo_DOUT, LOW);//TDO_DOUT.write(LOW);

      //Read the known chain length, or the full limit during discovery.
      for(uint8_t padIndex = 0; padIndex < maxControllers; ++padIndex) {
        ...
        for(uint8_t i = 0; i < 16; ++i) {
          const uint8_t dataIn = gpio_get(tdo_DIN);//TDO_DIN;
      ...
      gpio_put(tdo_DOUT, HIGH);//TDO_DOUT.write(HIGH);
  ...
  public:
    ThreedoPort(uint8_t PIN_CLK, uint8_t PIN_DOUT, uint8_t PIN_DIN)
      : tdo_CLOCK(PIN_CLK), tdo_DOUT(PIN_DOUT), tdo_DIN(PIN_DIN) {
    }
    void begin() {
      
      //Init pins
      pinMode(tdo_CLOCK, OUTPUT); digitalWrite(tdo_CLOCK, HIGH);//TDO_CLOCK.config(OUTPUT, HIGH);
      pinMode(tdo_DOUT, OUTPUT); digitalWrite(tdo_DOUT, HIGH);//TDO_DOUT.config(OUTPUT, HIGH);
      pinMode(tdo_DIN, INPUT_PULLUP);//TDO_DIN.config(INPUT, HIGH);
      delayMicroseconds(100);
    }

    void __not_in_flash_func(update)(const uint8_t maxControllers = THREEDO_MAX_CTRL) {
      //keep last data
      ...
      readControllers(min(maxControllers, static_cast<uint8_t>(THREEDO_MAX_CTRL)));
