...
class ThreedoPort {
  private:
    ...
    void doClockCycle() {
      ...
      digitalWrite(tdo_CLOCK, HIGH);//TDO_CLOCK.write(HIGH);
      delayMicroseconds(4);
      digitalWrite(tdo_CLOCK, LOW);//TDO_CLOCK.write(LOW);
      delayMicroseconds(4);
    }
    ...
    void readControllers() {
      ...
      digitalWrite(tdo_DOUT, LOW);//TDO_DOUT.write(LOW);
      ...
      for(uint8_t padIndex = 0; padIndex < THREEDO_MAX_CTRL; ++padIndex) {
        ...
        for(uint8_t i = 0; i < 16; ++i) {
          const uint8_t dataIn = digitalRead(tdo_DIN);//TDO_DIN;
      ...
      digitalWrite(tdo_DOUT, HIGH);//TDO_DOUT.write(HIGH);
