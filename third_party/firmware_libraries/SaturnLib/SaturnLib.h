...
#include "MegadriveSixButtonDecode.h"
...
class SaturnPort {
  private:
    static constexpr uint8_t MEGADRIVE_TH_SETTLE_US = 8;
...
//    #endif
    ...
    void __not_in_flash_func(readMegadrivePad)(uint8_t nibble_0, uint8_t nibble_1) {
      ...
      // If the initial page claims Left+Right together, retain the previous
      // base buttons but continue the extension probe and miss accounting.
      const bool basePageValid =
          (nibble_0 & 0b00001100) != 0b00000000;
      if (basePageValid) {
        // RLDU
      ...
      for (uint8_t i = 0; i < 4; i++) { //set TH high-low-high-low
        setTH(th);
        delayMicroseconds(MEGADRIVE_TH_SETTLE_US);
        th ^= 1;
      }
     
      nibble_0 = readMegadriveBits();
      // The connected OEM MK-1653 leaves D1 high on its ID page (0x2).
      // Treat 0x0 and 0x2 as candidates, then require the fixed marker page
      // below so a three-button pad with Up held cannot qualify.
      const bool sixButtonIdPhase =
          saturnlib_megadrive::six_button_id_phase(nibble_0);
      uint8_t sixButtonExtraPage = 0x0F;
      uint8_t sixButtonMarkerPage = 0;
      bool sixButtonSignature = false;

      if (sixButtonIdPhase) {
        setTH(HIGH);
        delayMicroseconds(MEGADRIVE_TH_SETTLE_US);
        sixButtonExtraPage = readMegadriveBits();

        setTH(LOW);
        delayMicroseconds(MEGADRIVE_TH_SETTLE_US);
        sixButtonMarkerPage = readMegadriveBits();
        sixButtonSignature = saturnlib_megadrive::six_button_marker_valid(
            sixButtonMarkerPage,
      ...
      // A worn connector or an OEM pad can intermittently miss a complete
      // extension response. Require several marker-validated signatures, but
      // allow bounded misses between them. Once confirmed, retain the subtype
      // and last valid extension page until physical disconnect.
      ...
      } else if (!sc.sixbtn_confirmed && sc.sixbtn_signature_hits > 0) {
      ...
      if (sixButtonSignature && sc.sixbtn_confirmed) {
        ...
        // SA1111
        // 8BitDo M30 marker layout: ...S.H (Star and Home, active low).
        #ifdef SATLIB_ENABLE_8BITDO_HOME_BTN
          setControlValues(
              sc, 3,
